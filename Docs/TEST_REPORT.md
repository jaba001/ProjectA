...
| 상점 인카운터 / J | Editor 컴파일 완료, 작동 검증 미실행. 선택·저장·협동 테스트 코드 추가/갱신 |
...
| A5 | 첫 전투 승리 → Continue → 상점 선택 → 나가기 → 두 번째 Combat 노드 선택 | 같은 Gameplay에서 다음 전투가 시작되고 파티·HP·노드 진행이 유지됨 | 대기 |
...
목적: 첫 전투 뒤 3개 선택지 중 하나의 상점에 진입·퇴장하고 다음 전투로 이어지는 최소 인카운터 흐름을 확인한다. 대상은 새 Run의 선택 상태·CommonUI 화면·저장·Host 진행/Client 표시다. 모든 J 항목은 미실행이다.

준비: 최신 Development Editor 빌드를 사용한다. 에디터가 이전 코드를 사용 중이면 작업 저장 후 재시작하고 MainMenu에서 새 Run을 만든다. 기존 저장은 상점 없는 원래 경로를 유지한다. 협동은 I의 독립 게임 창·개발용 방 절차를 사용한다. 상품·재화·회복 효과·확률 추첨은 이번 대상에 없다.

| ID | 실행 절차 | 기대 결과 | 상태 |
|---|---|---|---|
| J1 | 새 Run의 첫 전투 승리 → Continue | 상점1·상점2·상점3의 텍스트 버튼 3개 표시. 두 번째 전투로 직접 건너뛸 수 없음 | 미실행 |
| J2 | 상점2 선택 → 이름 확인 → 나가기 → 두 번째 Combat 노드 선택 | 상점2 이름·나가기 버튼 표시. 퇴장 후 다음 전투 가능, HP·소유권 유지. 다른 상점 추가 방문 불가 | 미실행 |
| J3 | 새 Run을 각각 만들어 상점1·상점3도 선택·퇴장 | 선택한 이름과 진입 화면이 일치하고 모두 같은 퇴장 흐름 제공 | 미실행 |
| J4 | 선택 화면 또는 상점 내부에서 게임 종료 → MainMenu Continue | 선택 목록 또는 선택한 상점 복원. 퇴장 저장 후 재개하면 다음 전투 지도 복원 | 미실행 |
| J5 | 두 번째 전투 승리 → Continue, 별도 Run에서 첫 전투 패배 | 마지막 승리 뒤 Run 완료. 패배 뒤 상점·다음 전투 진입 불가 | 미실행 |
| J6 | 개발용 2인 협동에서 J1·J2 진행. Client 버튼·진행 권한 확인 | 같은 목록·선택한 상점·퇴장 상태 표시, Host만 결정. 다음 전투에서 각자 자기 캐릭터 조작 | 미실행 |
| J7 | 별도 보관한 상점 도입 전 저장으로 이어하기 | 원래 두 전투 경로 유지. 상점 상태를 추정 추가하지 않음 | 미실행 |
| J8 | Session Frontend → Automation에서 `ProjectA.Run.Encounter.` 실행 | 3개 선택·잘못된/중복 요청·저장 실패 복구·재개·기존 경로 호환 통과 | 미실행 |
...
실패 시 J 번호·선택한 상점·첫 실패 화면·로그를 기록한다. 저장 오류가 표시되면 진행이 유지되는지 확인하고 원인을 해소한 뒤 같은 선택/나가기 버튼으로 재시도한다. 관리 저장·승계의 상점 내부 재개는 별도 개발 fixture 검증 대상이며 일반 협동 방의 주소 참가로 기존 Run을 재개하지 않는다.
