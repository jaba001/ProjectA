...
갱신일: 2026-09-13. 최신 실행 결과와 사용자 검증 절차를 관리한다. 이력은 [HISTORY](HISTORY.md), 설정은 [PROJECT_PLAN](PROJECT_PLAN.md)를 따른다.
...
| ID | 절차 | 기대 결과 | 상태 |
|---|---|---|---|
| 1-1 | MainMenu → 새 게임 → 캐릭터 1~4명 생성 → 시작 → 첫 Combat 노드 선택 | Gameplay에 생성한 파티가 등장하고 현재 턴·HP/AP가 표시됨 | 대기 |
| 1-2 | 자기 턴에 Move 선택 → 표시된 빈 도달 타일 클릭 | 실제 이동·점유 타일·자원이 갱신되고 완료 후 다음 입력이 가능함 | 대기 |
| 1-3 | 사용 가능한 스킬 선택 → 유효한 적/타일 클릭 | 타겟 표시와 실제 효과·HP/AP가 일치하고 스킬 완료 후 입력이 풀림 | 대기 |
| 1-4 | 적 턴과 이동/스킬 처리 중 Move·스킬·타일 클릭을 시도하고 정상 진행을 기다림 | 유효하지 않은 행동은 실행되지 않고 적 행동 완료 후 자기 턴에 입력이 다시 가능함 | 대기 |
| 1-5 | 첫 전투 승리 → Continue → 상점 선택 → 나가기 → 두 번째 Combat 노드 선택 | 같은 Gameplay에서 다음 전투가 시작되고 파티·HP·노드 진행이 유지됨 | 대기 |

실패 시 최초 단계·화면·로그를 기록한다. 1-2/1-3의 입력 문제는 버튼 선택·범위 표시·이동/효과 적용 단계로 구분한다.

## 2. 승계 검증

사용자 명시 요청으로 2-1~2-3을 실행했다. 재현 시 작업 저장·PIE 종료 후 프로젝트 루트 PowerShell에서 실행한다. 보고서는 실행 시각별 경로에 저장한다.
...
| 9-2 | 각 창에서 자기 턴의 이동·회복약·스킬·턴 종료, 타인의 유닛 조작 시도 | 대기실 종료 후 실제 마우스 입력 작동. 본인 캐릭터만 조작, Turn·Grid·HP/AP·사망 상태 일치 | 미실행 |
...
| 9-8 | 새 주소 검증 테스트와 필요한 기존 2/4인·관리 승계 fixture 실행 | 주소의 경로/옵션 주입 거절. 개발 방 옵션이 없는 기존 전투·승계 흐름 유지 | 미실행 |
...
개발 확인: `Saved/Automation/RunShopBuildFinal2.log`의 Development Editor / Win64 컴파일 성공. 선행 컴파일에서 검증 헬퍼 include 경로와 지역 변수 이름 충돌을 수정했다. 최종 컴파일 오류·경고 0. 자동화 소스는 컴파일만 수행했으며 게임·PIE·자동화 테스트를 실행하지 않았다. 기존 진행/저장/소유권/확정 턴/관리 테스트와 단독·2/4인·승계 PIE fixture는 상점 단계를 반영했으나 최신 코드의 성공 결과는 아직 없다.
