...
이 API는 실제 불참 판정·Host 승계 승인을 대신하지 않는다. 기본 Run은 Human을 유지하며 끊김만으로 AI를 켜지 않는다. 기존 v3 협동 복구는 원래 참가자 연결 조건을 유지한다. 6번 관리 v4는 Host가 승인한 인간 목록을 전투 밖에도 저장하고 현재 인간에게만 연결을 배정한다. 재개 시 인간 목록을 늘리지 못하므로 이미 AI인 소유자는 그 Run에서 인간으로 복귀할 수 없다. 각 새 전투와 복원은 이 목록으로 모드를 결정하며 준비 API에서도 목록과 다른 모드를 거절한다. 메뉴·승계·로컬 실행 권한의 한계는 [승계 구현 메모](T14_RESUME.md)를 따른다. MMR 세부 정책은 별도 미정이다.

## 저장 호환

일반 Run SaveGame의 전투 저장은 v3, 관리 Run은 v4이며 전투 본문은 schema 2로 기록한다. 각 유닛의 `PartyControlMode`를 저장하며, 이 필드는 파티 유닛의 조작 방식이다. v4는 바깥 Participation과 안쪽 모드의 일치도 검사한다. 적 유닛의 기존 AI는 변경하지 않는다.
