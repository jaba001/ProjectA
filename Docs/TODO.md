...
최근 정리: 2026-09-10 · `main` · T14 순차 1~5번 완료 / 6번 기반 검증 완료·승계/동의/복귀 정책 답변 대기 / 7~8번 순차 대기.

[게임 기획 방향](GAME_DESIGN.md) · [기획·구현 현황](PROJECT_PLAN.md) · [코드 리뷰와 검증 시나리오](CODE_REVIEW.md)

## 다음에 켜면 여기부터

[T14 순차 작업 대기열](T14_QUEUE.md): 사용자 요청에 따라 1~8번을 한 번에 한 개씩 진행한다. 1~4번의 소유권·서버 명령·2인 동기화·확정 턴 저장과 기존 Host 복구에 이어 5번 아군 AI 판단·행동도 검증을 완료했다. 5번은 전체 회귀 56건, 실제 AI Co-op과 독립 프로세스 AI 저장·복원을 통과했다. 번호별 검증·커밋·push를 마친 뒤 다음 번호로 자동 진행하며 다음은 6번 명시적 Host 승계·AI 이어하기, 7~8번은 대기한다. 기획 결정이 필요하면 선택지별 장단점을 사용자에게 확인한다. 전투 밖 노드 선택·Continue의 협동 결정권은 답변 대기 중이다.
...
**T14 로컬 상대 Snapshot/Unreal USaveGame v1 전투 사이클을 검증했다. Co-op은 최대 4인·본인 캐릭터 소유권 고정·기존 Host 유지와 명시적 승계/AI 이어하기로 기획을 확정했다. AI 전환은 시작 시 사전 동의, 이후 MMR은 인간 참가자만 반영, 복구는 확정 턴 경계를 기준으로 한다. 순차 1~5번의 소유권·서버 명령·2인 동기화·기존 Host 턴 복구·아군 AI를 검증했다. 다음 6번 명시적 승계·불참 AI 이어하기와 7~8번 3~4인·온라인 서비스는 후속 범위다. 기본 Run은 싱글플레이를 유지하고 Unreal 기본 기능을 우선한다.** 기존 slice 실행 결과는 [Vertical Slice 보고](VERTICAL_SLICE_REPORT.md), 후속 검증은 각 작업 카드와 [Listen Server 안내](T14_NETWORK.md), 에디터 연결은 [설정 안내](VERTICAL_SLICE_SETUP.md)를 기준으로 한다.
...
- [ ] 순차 7~8번 3·4인 확장과 온라인 인증·Snapshot·MMR 권위
