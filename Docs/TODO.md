...
최근 정리: 2026-09-10 · `main` · T14 순차 1~5번 완료 / 6번 로컬 승계·싱글 전환 검증 완료 / 7번 독립 3·4인 검증 완료·승계 통합 대기 / 8번 Steam 우선·무료 개발 환경 준비 필요.

[게임 기획 방향](GAME_DESIGN.md) · [기획·구현 현황](PROJECT_PLAN.md) · [코드 리뷰와 검증 시나리오](CODE_REVIEW.md)

## 다음에 켜면 여기부터

[T14 순차 작업 대기열](T14_QUEUE.md): 사용자 요청에 따라 1~8번을 한 번에 한 개씩 진행한다. 1~5번과 추가 승인된 7번 독립 3·4인 전투·기존 Host 복구를 검증했다. 번호별 검증·커밋·push 뒤 다음 번호로 진행한다. 후속 답변으로 최초 번호순 Host 승계·사전 동의 없는 Host 단독 AI 전환·해당 Run의 AI 유지(3C)·Steam 우선 4B·Host 노드/Continue(5A)를 확정했다. 확정한 선택을 다시 질문하지 않으며 남은 정책이 실제 필요한 단계에서 장단점을 확인한다.

6번은 관리 Run v4에 인간 참가 목록을 전투 밖에도 저장하고 로컬 권위 저장소·싱글 전환 메뉴·새 Host 전투 복구를 연결했다. 최신 stamp·단일 실행 lease·번호순 Host·이미 AI인 계정의 인간 복귀 거절을 검사하며 일반 v1/v2/v3 경로와 분리한다. 현재 실행 검증과 진입 조건은 [재개 구현 메모](T14_RESUME.md)를 따른다. C++ 개발 호출자·공유 로컬 저장소 통합을 실제 Steam 인증·원격 중앙 저장 완료로 취급하지 않는다.

사용자의 `7,8도 진행해줘` 추가 승인으로 당시 6번 대기와 독립적인 7번 3·4인 전투·끊김·기존 Host 복구를 검증했다. 최종 Editor 빌드·전체 65건과 별도 2·3·4인 AI 복구 3건을 통과했고 일반 입장 정원은 엔진 GameSession으로 4명에 맞췄다. 승계/불참 AI 통합은 6번 구현 후 별도 검증한다.

8번은 [온라인 준비 문서](T14_ONLINE.md)에 공식 연동 지점·인증/저장 책임·실제 수용 기준을 정리했다. Steam+PlayFab을 선택했지만 사용자는 서버 운영비를 피하고자 한다. Steam P2P 전투와 중앙 저장·MMR 비용을 구분하고 무료 개발 한도·서비스 준비 상태를 확인한다. 실제 온라인 코드·배포·MMR은 미완료이며 미정인 랭크 정책이나 유료 리소스를 임의로 확정하지 않는다.

추가 완료 점검에서 협동·계정 기반 저장을 메인 메뉴가 싱글플레이로 열어 복구 실패·조작 불가가 되는 경로를 수정했다. 버튼 표시와 실제 로드 모두 같은 지원 조건을 검사하며 기존 협동 로드 API는 유지한다. Editor 빌드·전체 67건을 통과했다. 이 일반 Continue 제한을 임의로 풀지 않고 명시적 전환 경로를 별도로 연결한다.
...
**T14 로컬 Snapshot 전투와 순차 1~5번, 7번 독립 3·4인 전투/기존 Host 복구를 검증했다. Co-op은 최대 4인·본인 소유권 고정·번호순 명시적 승계·Host 단독 AI 전환·해당 Run의 AI 유지·인간 참가자만 MMR·확정 턴 복구로 결정했다. 6번은 로컬 개발 저장소의 승계·싱글 전환을 검증했고 7번의 4→3인 승계 확장과 8번 실제 서비스는 남아 있다. 기본 Run은 싱글플레이를 유지하고 Unreal 기본 기능을 우선한다.** 기존 slice 결과는 [Vertical Slice 보고](VERTICAL_SLICE_REPORT.md), 후속 검증은 [Listen Server 안내](T14_NETWORK.md), 에디터 연결은 [설정 안내](VERTICAL_SLICE_SETUP.md)를 따른다.
...
### T14 · 하이브리드 멀티플레이 기반 설계 — 로컬 Snapshot·2인 동기화·턴 복구·아군 AI 검증 완료 / 승계 후속
...
네트워크 transport는 Steam P2P를 우선한다. 선택한 Steam+PlayFab 조합의 무료 개발 한도와 출시 운영비는 [온라인 안내](T14_ONLINE.md)에서 구분한다.
...
- 참가 번호와 승계: 최초 Host 1번, 최초 합류 순서대로 2·3·4번을 고정한다. 명시적 재개에 참여하는 인간 중 가장 작은 번호가 Host다. 2·3·4번이 혼자 싱글로 전환하면 본인이 Host가 된다.
- AI 전환: 개인별 사전 동의 없이 Host가 단독 결정한다. 싱글 전환 시 나머지 캐릭터는 해당 Run 종료까지 AI를 유지하며 원래 소유자가 돌아와도 인간 조작으로 복귀하지 않는다(3C).
- 진행 결정권: 현재 Host만 다음 노드와 승리 후 Continue를 확정한다(5A).
- MMR: 랭크 시스템 도입을 목표로 한다. AI 전환 후 이어간 결과는 현재 인간 참가자에게만 반영하고 불참자에게 추가 변동을 주지 않는다.
- 복구: 마지막으로 확정된 턴 경계에서 이어한다. 순차 4번의 v3 전투 저장과 기존 Host 새 세션 복구로 구현했으며 명시적 Host 승계는 6번이다.
...
기본 Vertical Slice와 Run은 싱글플레이를 유지한다. 순차 1~5번의 2인 동기화·확정 턴 복구·아군 AI 판단과 행동을 구현·검증했다. 6번은 관리 v4의 명시적 승계·참여 영속화·싱글 전환 메뉴와 복구를 연결한다. 기존 v3는 원래 참가자 전원, 승인된 v4는 현재 인간 참가자만 연결을 요구한다. 실제 온라인 기능 전체를 완료한 것으로 취급하지 않는다.
...
- AI 전환 뒤 원래 소유자의 관전 허용 여부와 비전투 장비·스킬·Ready 처리. 해당 Run에서 인간 조작 복귀는 허용하지 않음
- 현재 인간 참가자를 MMR 정산 대상으로 확정하는 시점과 MMR 계산·정산 단위
- 고의 이탈·미확정 턴 반복·승계 후 중복 진행 방지의 구체적인 랭크 정책
- Steam 개발 환경과 PlayFab 무료 개발 한도·출시 운영비 조건
- Lobby / Invite 방식
...
확정 턴 저장과 기존 Host 복구는 4번에서 검증한다. 5번은 소유권을 유지하는 아군 AI의 실제 행동·인간 입력 차단·저장 모드 복구를 검증한다. 6번 승계/AI 이어하기는 원래 참가자만 재개 가능, 번호순 Host 선정, Host 단독 AI 전환, 타인 인간 조작·이미 AI인 계정의 인간 복귀 거절, 하나의 유효 Host와 중복 재개 방지를 확인한다. 최대 4인 지원 완료 전 7번에서 3·4인도 검증하며, 온라인 결과 1회 반영·MMR 신뢰는 8번에서 검증한다. 로컬 테스트용 신원·저장소 검증을 온라인 인증·랭크 보안 완료로 기록하지 않는다.
...
- [x] 순차 4번 확정 턴 체크포인트·저장 실패 재시도·기존 Host 새 세션 복구와 원래 소유권 보존
- [x] 순차 5번 아군 AI 판단·행동·별도 명령 세션·모드 저장/복구 — 전체 회귀·실제 AI Co-op·독립 프로세스 AI 저장/복원 통과
- [x] 순차 6번 로컬 개발 범위: 명시적 Host 승계·영구 AI·싱글 전환 UI·현재 Human 연결·중복 재개 거절, 실제 3→2인/4→1인 복구 검증
- [x] 순차 7번 독립 범위: 실제 3·4인 전투·끊김·기존 Host 새 세션 복구 및 연결된 소유자의 AI 모드 검증
- [ ] 순차 7번 후속: 6번 정책·구현 이후 3·4인 승계/불참 AI 통합
- [ ] 순차 8번 온라인 인증·중앙 저장·Snapshot·MMR 권위
...
`AssignRunParticipant`와 `ApplyCombatParticipantBindings`는 신뢰된 서버 C++ 연결 배정이며 실제 로그인 인증이 아니다. 후속 5A 확정으로 현재 Host만 노드·Continue UI와 실제 요청을 실행하며 2·3·4인 자동화도 Host 버튼을 사용한다. 네트워크 최종 유닛은 결과 화면에서 유지한 뒤 명시적인 Continue 또는 월드 종료에서 정리한다. 최초 3번의 Development Editor / Win64 빌드·전체 자동화 42건·별도 Snapshot 상대 PIE 1건과 이후 확장 결과는 [대기열](T14_QUEUE.md)에 구분해 기록한다.

4번은 v3 확정 턴 저장·실패 재시도·기존 Host의 새 세션 복구를 구현했다. 원래 소유권·턴·HP/AP/SubAP·재고·사망·점유·실제 장착과 고정 상대를 보존하고 새 명령 실행 ID를 부여한다. Editor 빌드·전체 52건(성공 35·경고 동반 성공 17·실패 0), 독립 프로세스 Writer/Reader·원본 상대 교체/삭제 복구·별도 Snapshot 전투 PIE를 통과했다. 지원하지 않는 GAS 상태, 손상과 배치 불일치는 거절한다. [확정 턴 저장·복구](T14_CHECKPOINT.md)에 저장 경계와 현재 제한을 정리한다.

5번은 PlayerUnit의 `UPartyAutoCombatComponent`가 턴·완료 통지를 받아 회복약·스킬·이동·턴 종료를 선택한다. Human/ServerAI 모드는 소유권·팀과 분리하며 전투 시작 전 신뢰된 서버 C++ 전환만 허용한다. 후속 사용자 답변에 따라 개인 사전 동의 검사를 제거하고 기존 동의 필드는 저장 구조 호환용으로 유지한다. AI는 가짜 PlayerController 없이 별도 실행 세션·순번과 공통 행동 검증을 사용하고 AI 유닛의 인간 요청을 거절한다. Run 저장 v3의 전투 본문 schema 2에 모드를 보존하며 schema 1은 Human만 허용한다. 복원 시 새 AI 실행 세션을 부여한다.

5번 검증: Development Editor / Win64 최종 Build5, 전체 자동화 56건(성공 36·경고 동반 성공 20·실패 0), `Saved/Automation/T14PartyAIFull2/index.json`. 실제 AI opt-in `Coop2` 1건과 독립 프로세스 `RestartWrite`·`RestartRead` 각 1건, 별도 Snapshot PIE 회귀 1건도 통과했다. 초기 `Coop1`의 테스트 생존 전제 보정과 상세 결과는 [대기열](T14_QUEUE.md)에 기록한다. 원래 참가자 전원 연결 조건은 유지한다. 추가 승인으로 7번 독립 3·4인 검증도 최종 Build3·전체 65건·별도 AI 복구 3건을 통과했다(`T14ScaleFull1/index.json`, `T14ScaleAI1/index.json`). 6번 승계/불참 AI·7번 해당 통합·8번 실제 온라인 서비스와 T14 전체 완료는 체크하지 않는다.
...
| 날짜 | 작업 | 완료/검증 | 다음 시작점 |
|---|---|---|---|
| 2026-09-10 | T14 순차 5번 아군 AI 판단·행동 | Editor Build5·전체 56건(성공 36·경고 동반 성공 20·실패 0), 실제 AI Coop2·독립 프로세스 AI RestartWrite/RestartRead·Snapshot PIE 회귀 각 1건 통과 | 순차 6번 명시적 Host 승계·AI 이어하기, 7~8번 대기 |
