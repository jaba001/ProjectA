...
최근 정리: 2026-09-10 · `main` · T14 로컬 Snapshot 구현, Co-op 소유권·Host 승계·AI 이어하기 기획 확정 / 구현 후속.
...
**T14 로컬 상대 Snapshot/Unreal USaveGame v1 전투 사이클을 검증했다. 후속 Co-op은 최대 4인·본인 캐릭터 소유권 고정·기존 Host 유지와 명시적 승계/AI 이어하기로 기획을 확정했다. AI 전환은 시작 시 사전 동의, 이후 MMR은 인간 참가자만 반영, 복구는 확정 턴 경계를 기준으로 한다. 다음 구현은 소유권과 서버 명령 검증·2인 동기화이며, 승계·AI·턴 복구는 후속 단계로 나눈다. 기본 Run은 싱글플레이를 유지하고 Unreal 기본 기능을 우선한다.** 기존 slice 실행 결과는 [Vertical Slice 보고](VERTICAL_SLICE_REPORT.md), 후속 검증은 각 작업 카드, 에디터 연결은 [설정 안내](VERTICAL_SLICE_SETUP.md)를 기준으로 한다.
...
이번 후속 변경은 확정 기획과 구현 경계를 문서화한 작업이며 C++·기존 SaveGame v1·에셋 변경은 없다. 문서 내용·로컬 링크·스테이징 diff를 검증하며 위 32건/별도 PIE 결과는 이전 로컬 Snapshot 구현의 기록이다. 빌드·다중 PIE·패키지 재빌드를 새로 수행한 것으로 기록하지 않는다.

다음은 원래 소유권·Run 참가자 식별과 서버 요청 검증을 정의하고 2인 Listen Server 상태 복제를 구현하는 단계다. 이후 턴 복구·승계·아군 AI를 순차 검증한다. 랭크의 이탈 방지 등 미결정 정책을 임의로 확정하지 않으며 T14 전체 완료는 체크하지 않는다.
