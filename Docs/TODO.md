...
최근 정리: 2026-09-10 · `main` · T14 순차 1~5번 완료 / 6번 정책 답변 대기 / 7번 독립 3·4인 검증 완료·승계 통합 대기 / 8번 준비 문서 완료·서비스 선택 대기.

[게임 기획 방향](GAME_DESIGN.md) · [기획·구현 현황](PROJECT_PLAN.md) · [코드 리뷰와 검증 시나리오](CODE_REVIEW.md)

## 다음에 켜면 여기부터

[T14 순차 작업 대기열](T14_QUEUE.md): 사용자 요청에 따라 1~8번을 한 번에 한 개씩 진행한다. 1~4번의 소유권·서버 명령·2인 동기화·확정 턴 저장과 기존 Host 복구에 이어 5번 아군 AI 판단·행동도 검증을 완료했다. 번호별 검증·커밋·push를 마친 뒤 다음 번호로 자동 진행하되, 추가 승인에 따라 6번 정책 대기 중 7번 독립 범위를 먼저 진행했다. 세부 현황은 아래와 대기열을 따른다. 기획 결정이 필요하면 선택지별 장단점을 사용자에게 확인한다. 전투 밖 노드 선택·Continue의 협동 결정권은 답변 대기 중이다.

6번은 참여 DTO와 로컬 권위 저장소 대역의 기반 구현·빌드·전체 회귀 60건과 별도 프로세스의 중복 실행 거절/종료 후 명시적 획득을 검증했다. 승계 확정 주체·미동의 참가자의 Run 시작·AI 소유자의 복귀 시점에 관한 세 질문을 보냈으며, 답변 전 해당 정책과 UI/실제 승계 복구를 확정하지 않는다. [재개 구현 메모](T14_RESUME.md)에 현재 미연결 기반과 후속 통합 지점을 기록한다. 기반 커밋을 6번 완료로 취급하지 않으며 같은 질문은 상태 변화 없이 반복하지 않는다.

사용자의 `7,8도 진행해줘` 추가 승인으로 6번 답변 대기와 독립적인 7번 3·4인 전투·끊김·기존 Host 복구를 검증했다. 최종 Editor 빌드·전체 회귀 65건과 별도 2·3·4인 AI 복구 3건을 통과했고 일반 입장 정원은 엔진 GameSession으로 4명에 맞췄다. 해당 변경의 커밋·push 뒤 8번을 이어가며, 승계/불참 AI 통합은 6번 구현 후 별도 검증한다. 8번 플랫폼(EOS/Steam)과 Backend(PlayFab/직접 HTTPS 서버·PostgreSQL) 선택 질문을 보냈다. 서비스나 미결정 MMR 정책을 임의로 정하지 않는다.

7번 `152aed7`의 push 후 8번 공식 서비스 연동을 조사하고 [온라인 준비 문서](T14_ONLINE.md)에 선택지·필요한 개발 서비스·인증/저장 책임과 실제 수용 기준을 기록했다. 현재는 플랫폼·Backend 답변 대기이며 실제 온라인 코드·배포·MMR 반영은 미구현이다. 준비 문서 커밋을 8번 완료로 취급하지 않고 상태 변화 없이 질문이나 조사·커밋을 반복하지 않는다.

추가 완료 점검에서 협동·계정 기반 저장을 메인 메뉴가 싱글플레이로 열어 복구 실패·조작 불가가 되는 경로를 수정했다. 버튼 표시와 실제 로드 모두 같은 지원 조건을 검사하며 기존 협동 로드 API는 유지한다. Editor 빌드·전체 67건(신규 이어하기 회귀 2건 포함)을 통과했다. 승계 정책 세 항목과 서비스 조합·노드/Continue 결정권은 선택지로 다시 요청한 상태이며 답변 전 T14 전체를 완료 처리하지 않는다.
...
**T14 로컬 상대 Snapshot/Unreal USaveGame v1 전투 사이클과 순차 1~5번, 7번 독립 3·4인 전투/기존 Host 복구를 검증했다. Co-op은 최대 4인·본인 캐릭터 소유권 고정·기존 Host 유지와 명시적 승계/AI 이어하기로 기획을 확정했다. AI 전환은 시작 시 사전 동의, 이후 MMR은 인간 참가자만 반영, 복구는 확정 턴 경계를 기준으로 한다. 6번 명시적 승계/불참 AI와 해당 7번 통합, 8번 실제 온라인 서비스는 미완료다. 기본 Run은 싱글플레이를 유지하고 Unreal 기본 기능을 우선한다.** 기존 slice 실행 결과는 [Vertical Slice 보고](VERTICAL_SLICE_REPORT.md), 후속 검증은 각 작업 카드와 [Listen Server 안내](T14_NETWORK.md), 에디터 연결은 [설정 안내](VERTICAL_SLICE_SETUP.md)를 기준으로 한다.
...
확정 턴 저장과 기존 Host 복구는 4번에서 검증한다. 5번은 소유권을 유지하는 아군 AI의 실제 행동·인간 입력 차단·동의와 저장 모드 복구를 검증한다. 6번 승계/AI 이어하기는 원래 참가자만 재개 가능, Host가 바뀌어도 타인 인간 조작 거절, 사전 동의와 불참 상태의 명시적 전환, 하나의 유효 Host와 중복 재개 방지를 확인한다. 최대 4인 지원 완료 전 7번에서 3·4인도 검증하며, 온라인 결과 1회 반영·MMR 신뢰는 8번에서 검증한다. 로컬 테스트용 신원·저장소 검증을 온라인 인증·랭크 보안 완료로 기록하지 않는다.
...
5번은 PlayerUnit의 `UPartyAutoCombatComponent`가 턴·완료 통지를 받아 회복약·스킬·이동·턴 종료를 선택한다. Human/ServerAI 모드는 소유권·팀과 분리하며, 전투 시작 전 신뢰된 서버 C++ 전환은 원래 소유자의 `Granted`·정책 버전 1 동의를 요구한다. AI는 가짜 PlayerController 없이 별도 실행 세션·순번과 공통 행동 검증을 사용하고 AI 유닛의 인간 요청을 거절한다. Run 저장 v3의 전투 본문 schema 2에 모드를 보존하며 schema 1은 Human만 허용한다. 복원 시 새 AI 실행 세션을 부여한다.
