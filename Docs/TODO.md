...
최근 정리: 2026-09-10 · `main` · T14 순차 1~5번 완료 / 6번 정책 확정·통합 진행 / 7번 독립 3·4인 검증 완료·승계 통합 대기 / 8번 Steam 우선·무료 개발 환경 준비 필요.
...
6번은 참여 DTO·로컬 권위 저장소 기반과 별도 프로세스의 중복 실행 거절·명시적 획득을 검증했다. 후속 확정안을 반영한 번호 데이터/후보 조회·Host 진행 권한·AI 사전 동의 검사 제거 위에 실제 싱글 전환 UI·참여 상태 영속화·승계 복구를 연결한다. 현재 구현 경계는 [재개 구현 메모](T14_RESUME.md)를 따르며 기반 작업만으로 6번 전체를 완료 처리하지 않는다.
...
**T14 로컬 Snapshot 전투와 순차 1~5번, 7번 독립 3·4인 전투/기존 Host 복구를 검증했다. Co-op은 최대 4인·본인 소유권 고정·번호순 명시적 승계·Host 단독 AI 전환·해당 Run의 AI 유지·인간 참가자만 MMR·확정 턴 복구로 결정했다. 6번 실제 승계/불참 AI와 해당 7번 통합, 8번 실제 온라인 서비스는 미완료다. 기본 Run은 싱글플레이를 유지하고 Unreal 기본 기능을 우선한다.** 기존 slice 결과는 [Vertical Slice 보고](VERTICAL_SLICE_REPORT.md), 후속 검증은 [Listen Server 안내](T14_NETWORK.md), 에디터 연결은 [설정 안내](VERTICAL_SLICE_SETUP.md)를 따른다.
...
기본 Vertical Slice와 Run은 싱글플레이를 유지한다. 순차 1~5번의 2인 동기화·확정 턴 복구·아군 AI 판단과 행동을 구현·검증했다. 다음 6번에서 명시적 Host 승계·재개 시 조작 전환·불참자 연결 예외와 AI 이어하기 UI를 다루며 현재는 원래 참가자 전원의 연결 조건을 유지한다. 전체 게임의 온라인 기능을 완료한 것으로 취급하지 않는다.
...
- [ ] 순차 6번 명시적 Host 승계 + 불참자 AI로 계속하기, 전환/UI·연결 예외·중복 재개 방지
