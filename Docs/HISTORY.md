...
## 2026-09-11 명시적 요청에 따른 2인 전투 재검증

- `e90cd21`과 기존 사용자 삭제 3건을 보존한 작업 폴더에서 `ProjectA.Coop.ListenServerClientCombat` 1건을 직접 실행했다. 직전 성공한 Editor 빌드를 재사용했다.
- 서로 다른 NetDriver의 2개 PIE 월드에서 조작권·이동·아이템·스킬·중복 요청 거절·턴/HP/AP/Grid/사망·결과·두 전투 완료를 확인했다. 경고 동반 성공 1건, 실패/오류 0, 경고 4, 테스트 본체 14.998초, 종료 코드 0.
- 로그·경고·수용 범위는 [TEST_REPORT F](TEST_REPORT.md#f-2026-09-11-codex-직접-실행-2인-전투-동기화)에 기록했다. 수동 사용성·Steam/P2P·복구·승계·3/4인 재검증으로 확대하지 않는다. 일반 사용자 테스트 원칙을 변경하지 않았다.
