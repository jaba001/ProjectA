...
전투 밖 노드 선택·Continue는 사용자 선택 5A에 따라 현재 Host만 실행한다. `GameplayPlayerController::CanIssueRunCommands`는 Authority·로컬 연결을 확인하고 Listen Server에서는 `GameplayGameModeBase::CanControlRunFlow`로 해당 연결의 신뢰된 참가 배정과 현재 `Run.HostAccountId`를 비교한다. 버튼 활성화와 실제 요청이 같은 조건을 검사한다. 미배정·다른 Host 계정·Client 및 서버의 원격 Controller는 실행할 수 없다. 일반 Standalone 흐름은 유지하며 이 진행 권한이 타인 캐릭터 조작권을 주지는 않는다.

2·3·4인 전투 자동화는 실제 Host의 노드 버튼·승리 Continue 버튼을 사용해 두 번째 전투와 최종 Run 완료·정리까지 검사한다. Client 버튼과 직접 요청은 상태를 바꾸지 못한다. 실제 Steam 로그인·신뢰 배정은 후속 온라인 연동 범위다.
...
6번 관리 v4는 `ResumeManagedRun`이 확정한 Host·HostEpoch·인간 참가 목록과 로컬 실행 lease를 사용한다. GameMode는 현재 인간 원래 참가자만 연결에 배정하고 AI 소유자의 인간 연결을 거절한다. 인간 한 명이면 본인 Host의 Standalone, 둘 이상이면 Listen Server로 실행한다. 새 Encounter와 턴 복구는 영속 목록에 따라 AI를 적용한다. 정상 종료에서는 Actor·타이머·행동을 정리한 뒤 실행 lease를 반환한다.

`UCombatActionAuthority`는 관리 실행 여부를 Reset 후에도 유지하고 현재 Run 전체 Identity·Host·실행 SessionId·lease를 검사한다. 직접 실행을 닫은 뒤 남아 있는 과거 Authority의 인간/AI 명령·바인딩·모드 변경도 거절한다. 복구 대기 중 인간 입력은 막고 유효한 서버의 복구 준비와 첫 AI 턴만 허용한다. 실제 로그인·원격 계정 배정은 8번이며 현재 C++ 개발 fixture의 배정은 인증 증명이 아니다.

새 opt-in `ProjectA.ManagedRunPIE.`는 `-T14ManagedRunPIE`로 실행한다. 원래 3인→현재 인간 2인·AI 1인 승계와 원래 4번의 실제 메뉴 싱글 전환을 검증하도록 구성했다. 결과와 범위는 [승계 구현 메모](T14_RESUME.md), [대기열](T14_QUEUE.md)을 따른다. 아래는 기존 v3·원래 Host 복구의 독립 검증 기록이다.
...
`-T14CheckpointAI`는 세 체크포인트 테스트 모두에 적용한다. 끊김 검증 이후 별도 테스트 준비 과정에서 개인 동의가 없는 원래 게스트의 저장 모드를 AI로 지정한다. 원래 소유자도 새 세션에 연결한 상태로 실제 AI 행동·인간 입력 거절·전체 Client 상태 일치를 확인한다. 끊김이 자동 AI 전환을 일으키거나 불참자의 연결을 생략하는 기능은 포함하지 않는다.
