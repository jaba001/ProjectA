...
오른쪽 목록에서 조작할 아군 → 스킬 → 대상 유닛/공격 타일 → 필요한 접근 목적지를 선택하고 계획 적용한다. 자신에게 인간 조작이 허용된 모든 생존 아군을 지정한 뒤 준비 완료한다. 왼쪽에는 아군 계획과 라운드 시작에 고정된 적 의도를 표시한다. 해결 중에는 편집을 막고 경과 시간·행동 단계·HP·남은 투사체를 표시한다.

대상 목록은 `IsValidUnitTarget`을 사용한다. 유닛 공격은 살아 있는 적, Guard는 살아 있는 아군만 표시하며 Wait·GroundAttack은 유닛 대상을 사용하지 않는다. 같은 조작 유닛과 스킬 목록을 유지하는 동안 선택 대상이나 다른 소유 아군이 사망하여 목록이 갱신되어도 스킬 초안을 유지한다. 죽은 대상은 합법 대상 후보로 갱신한다. `CanPlanCommand`로 자원·타일·대상을 검사한 뒤 계획 적용을 허용하며, 자신에게 인간 조작이 허용된 모든 생존 유닛에 유효한 계획이 적용되어야 준비 완료할 수 있다.

2026-09-16 입력 보완은 최종 초안 보존 수정을 포함해 Editor 컴파일이 성공했다. 최종 코드·문서 정적 검사를 통과했으며 작동 확인은 [TEST_REPORT 17절](TEST_REPORT.md#17-계획-입력-검사와-장착-tile-공격-ai)을 따른다. 아이콘·모델·애니메이션·VFX·UI 최종 디자인은 [TODO 5절](TODO.md#5-디자인-확정-후-구현할-일)의 확정 후 반영한다.
...
메뉴·설정·개발용 협동·RunMap·상점·결과의 개별 `ScaleBox`·`DownOnly` 축소는 제거한다. 모든 화면에 공통 DPI를 한 번 적용하고 기존 중앙 정렬·좌우 고정·스크롤·바인딩을 유지한다. 화면 비율이 달라도 동일한 배율 공식을 사용한다.

### 10-2 전투 좌우 배치와 카메라

라운드 계획의 왼쪽·오른쪽 패널은 바깥 가로 여백을 0으로 설정하여 뷰포트 가장자리에 배치한다. 위아래 여백은 기존 12 UI 단위를 유지한다. 패널 내부 여백·스크롤과 중앙 월드의 유닛/타일 입력은 유지한다.
