...
`URunEncounterWidget`은 기존 Gameplay RunLayer에서 인카운터 선택·상점 구매 화면을 표시한다. `EncounterChoice`에서 Host가 스킬상점·아이템상점·상점3 중 하나를 선택한다. 스킬상점·상점3은 기존 본인 캐릭터·개인 골드·현재/최대 HP, 스킬 4종과 1G HP 전체 회복 버튼을 유지한다. 최대 HP는 서버의 직업 설정에서 가져온다. 각 인간 참가자가 본인만 구매하며 스킬 중복·만피 회복·잔액 부족·사망·구매 처리 중 버튼은 비활성화한다. 구매 결과는 요청자에게 표시하고 나가기는 Host가 결정한다. 인카운터 분류 태그로 화면을 구분하며 이전 저장의 기본 이름도 새 이름으로 표시한다. CommonUI 메뉴 입력과 native 구성을 유지하며 WBP 재생성은 필요 없다. [스킬상점 확인](TODO.md#2-19-비무장-시작과-스킬-상점)

새 Run의 아이템상점은 CSV의 한국어 `게임 내 이름`·가격·판매 완료 상태를 가진 아이템 5칸과 **리롤 1G**를 표시한다. 전체 295개에서 현재 진열 내 중복 없이 추첨하며 상품은 각 1G다. 구매하면 본인 보유 수량을 갱신하고 해당 슬롯을 비활성화한다. 리롤은 5칸을 새로 구성하고 이전 상품의 재등장을 허용한다. 잔액 부족·사망·요청 처리 중에는 구매·리롤을 막으며 서버의 저장 결과로 잔액과 진열을 갱신한다. 기존 저장은 저장 당시 아이템 이름을 유지하며 아이템상점 schema가 없는 이전 저장에는 새 Run 안내를 표시한다. [아이템상점 확인](TODO.md#2-31-아이템상점-시험)

모든 상점 진입 시 `UCharacterEquipmentPanel`·상품 영역·`UCharacterInventoryPanel`을 좌우 3열로 표시한다. 기본 너비는 300·620·430 UI 단위이며 긴 상품명은 줄바꿈하고 각 목록은 스크롤한다. 상품 카드는 분류/스킬 아이콘·이름·가격·구매 버튼으로 구성한다. 인카운터 선택 화면에는 인카운터 선택 영역만 표시한다. 조회 캐릭터는 `GetInventoryCharacterId`로 가져와 사망 후에도 장비·인벤토리를 표시하고, 구매와 장비 드래그는 서버의 생존 Human 검증을 따른다. 패널 내용은 [8-2절](#8-2-gameplay-인벤토리와-설정-단축키), 현재 사용자 확인은 [TODO 2-33](TODO.md#2-33-시작-장비와-상점-드래그-장착)을 따른다.
...
전장에서 **적 또는 스킬이 요구하는 타일 한 번 클릭 → 실제 장착 스킬 버튼 클릭 → 준비 완료** 순서로 행동한다. 스킬 버튼을 누르면 자원·대상·좌표를 검사하고 바로 계획 적용을 요청한다. 상단 중앙은 라운드·생존 수·준비 유닛 수, 우측 상단은 선택 대상·적 HP와 계획을 표시한다. 좌하단 파티 카드는 이름·조작 주체·HP/AP/SAP·계획·준비 상태를 표시하고 현재 조작 유닛을 금색 테두리로 구분한다. 하단 중앙은 조작 캐릭터·장착 스킬·적용된 행동·오류, 우하단은 이동 예약과 준비 버튼을 배치한다. 인간 조작 가능한 캐릭터가 여럿인 기존 파티는 아군 클릭 또는 조작 캐릭터 목록으로 전환한다. 카드는 정보 표시용이며 초상화·확정 턴 순서는 추가하지 않는다.

스킬 버튼은 선택한 캐릭터에게 실제 장착된 DA만 사용한다. 새 Run의 네 직업은 비무장 공격 1개로 시작하고 구매한 스킬을 저장된 장착 목록에서 표시한다. 전사 5개·나머지 4개인 이전 Blueprint 기본값은 이전 저장 fallback으로만 보존한다. 비무장 공격은 기존 기본공격의 표시명만 변경한 것이다. 무장착 자동 공격과 Encounter 진입 시 추가 스킬 추첨은 사용하지 않는다. `CanPlanCommand`가 거절하는 버튼은 비활성화하고 사유를 제공한다. 인간 조작이 허용된 모든 생존 유닛의 계획을 검사하며 스킬 미선택은 비용 없는 턴 넘기기로 허용한다. 선택한 스킬은 `스킬 선택 취소`로 해제할 수 있고 이동 예약은 유지된다. 미선택 상태와 파티 카드에는 턴 넘기기를 표시하며 이동만 예약했다면 SAP 1만 차감한다. [사용자 확인](TODO.md#2-22-스킬-미선택-준비-완료)
...
### 8-2 Gameplay 인벤토리와 설정 단축키

Gameplay의 전투·지도·상점·결과 화면에서 `I`로 인벤토리를 열고 다시 누르면 닫는다. `Esc`는 설정을 열며 인벤토리가 열려 있으면 설정으로 전환한다. 설정에서 `Esc`를 다시 누르면 닫고, 화면 변경 확인 중에는 먼저 이전 설정을 복구한다. 펼쳐진 선택 목록은 첫 `Esc`로 닫힌다. 설정 중 `I`는 설정을 닫거나 초안을 변경하지 않는다.

`UGameplayRootWidget`의 CommonUI 지속 액션과 별도 `UtilityLayer`를 사용한다. 창이 열리면 배경 UI와 전장 입력을 잠그고 닫으면 기존 화면으로 복귀한다. 창은 로컬 표시이며 진행 중인 전투·협동 세션을 일시정지하지 않는다. MainMenu의 기존 버튼·뒤로 가기는 유지한다.

`UInventoryWidget`은 상점과 같은 `UCharacterEquipmentPanel`·`UCharacterInventoryPanel`을 나란히 표시한다. 현재 참가자 캐릭터의 이름·직업·개인 골드·보유/장착 스킬과 아이템의 표시 이름·장착 상태를 조회한다. 새 Run은 시작 장비부터 CSV의 `게임 내 이름`을 사용하고 기존 저장은 저장 당시 이름을 유지한다. 같은 종류도 개별 사본으로 드래그하며 장착 사본과 가방 사본을 구분한다. 사망 후에도 조회할 수 있다. 명시 스킬 목록이 없는 이전 저장은 권위 Run의 실제 파티 목록으로만 해석하고, 협동 클라이언트는 확인 불가 안내를 표시한다.

장비창의 무기 2칸·투구·장갑·신발·갑옷·목걸이 각 1칸·반지 2칸은 실제 Run 장착 상태를 표시한다. 새 Run은 직업별 시작 장비를 장착하며 양손 장비는 두 슬롯에서 같은 사본으로 표시한다. 본인 생존 Human은 **상점 페이즈에서만** 아이템→장비 슬롯 장착·장비→가방 해제·호환 장비끼리 교체할 수 있다. 단계·권한·Revision·태그 조건을 서버가 재검사하며 저장 실패·잘못된 대상·취소 시 상태를 유지한다. 분류 미정 아이템은 보관 안내를 표시한다. 능력치·아이템 부여 스킬·소비는 미구현이며 기존 CommonUI 입력·DPI·테마를 유지한다. 사용자 확인은 [TODO 2-20](TODO.md#2-20-인벤토리와-설정-단축키)·[2-33](TODO.md#2-33-시작-장비와-상점-드래그-장착)을 따른다.

### 8-3 개인 골드 상시 표시

Gameplay 좌측 상단에 `보유 골드 10G` 형식의 작은 패널을 표시한다. 지도·상점·전투·결과에서 인벤토리와 같은 소유 캐릭터의 확정 잔액을 사용하며 구매·회복·리롤 저장 성공 또는 복제 뷰 도착 시 갱신한다. 실제 0G는 표시하고 캐릭터 미확정·Run 없음·협동 대기실에서는 숨긴다. 패널은 마우스 입력을 통과시키고 인벤토리·설정 레이어 아래에 배치한다. [사용자 확인](TODO.md#2-21-개인-골드-상시-표시)
...
- 장비·인벤토리는 원본 `Widgets/WB_Equipment`·`Widgets/WB_Bag`의 슬롯·제목 구성과 에셋 참조를 기준으로 개선한다. `GUI_Elements/Icon_frame`·`bar_button_mid`·`equipment_male`과 부위별 아이콘을 원본 경로에서 참조한다. 실루엣은 표시용 배경이며 선택한 몸체의 실제 모델이 아니다. 아이템 분류 아이콘은 카탈로그의 GameplayTag로 결정하고 내부 영역·빈 슬롯·보유 아이템의 공통 스타일을 분리한다.
...
- 중앙 패널은 [10절](#10-공통-dpi와-전투-화면-배치)의 프로젝트 공통 DPI를 사용한다. MainMenu의 메뉴와 명시적인 관리 이어가기 패널은 같은 가로 배치를 유지하며 기존 관리 이어가기 표시 조건을 유지한다. 상점·인벤토리의 다열 패널은 공간이 부족할 때만 `ScaleBox`의 `DownOnly`로 추가 축소한다.
...
| 상점 선택·상점 | 선택 목록·상점 이름·상품·나가기·안내. 상점 진입 시 장비·인벤토리 동시 표시, Host 진행 권한 유지 |
| 인벤토리 | 상점과 공통 장비·보유 아이템/스킬 패널, 부위 아이콘·아이템 수량·목록 스크롤. `I`·닫기·배경 입력 차단 유지 |
| 전투 결과 | Designer/native 경로의 배경·승리/패배 결과·Continue·메뉴 복귀. 결과별 기존 진행 조건 유지 |
| 라운드 계획 | 상단 라운드 요약·우측 대상/적 현황·좌하단 파티 카드·하단 중앙 장착 스킬·우하단 이동/준비. 전장 타일/유닛 클릭과 해결·이동 중 입력 잠금 |
...
모든 화면에 같은 공통 DPI 공식을 적용하고 각 화면의 정렬·스크롤·바인딩을 유지한다. 메뉴·설정·개발용 협동·RunMap·결과는 개별 배율을 추가하지 않는다. 상점 3열·인벤토리 2열은 화면 공간이 부족할 때에만 `ScaleBox`의 `DownOnly`로 축소하고 긴 목록은 내부 스크롤로 표시한다.

### 10-2 전투 배치와 카메라

제공된 전투 화면의 배치를 참고하여 전장 중앙을 비우고 정보를 화면 가장자리에 분산한다. 공통 DPI 적용 전 UI 단위 기준이며 개별 `ScaleBox`는 추가하지 않는다.

| 영역 | 배치와 크기 |
|---|---|
| 라운드 요약 | 상단 중앙, 너비 620·최대 높이 104·상단 여백 16. 입력 통과 |
| 대상·적 현황 | 우측 상단, 너비 340·최대 높이 280·상단 여백 152. 기존 협동 안내 공간 확보 |
| 파티 카드 | 좌하단, 너비 620·최대 높이 280. 아군별 정보 카드와 선택 테두리 |
| 선택 캐릭터·스킬 | 하단 중앙의 남은 너비·최대 높이 300. 실제 장착 스킬과 요청 상태 |
| 이동·준비 | 우하단, 너비 340·최대 높이 340. 기존 예약/취소·준비/취소 조건 유지 |

하단 세 패널은 아래쪽으로 정렬하고 화면 가장자리 여백 24·패널 사이 간격 16을 적용한다. 각 내용 패널은 독립 스크롤하며 긴 캐릭터 이름은 파티 카드에서 말줄임·툴팁을 사용한다. 어두운 반투명 배경·둥근 테두리·안쪽 여백 14×10으로 가독성을 확보한다. 전체 루트와 하단 가로 컨테이너의 빈 공간은 입력을 통과시키고 정보 스크롤·조작 패널은 배경 전장 클릭을 막는다. 저장 재시도 안내는 상단 128 아래에 배치하여 라운드 요약과 겹치지 않는다.
...
`FCharacterPartyDraft`가 직업 목록과 독립적인 4슬롯·생성·삭제·직접 조작 선택을 관리한다. 슬롯별 위젯 참조를 묶으며 기존 WBP 이름·이벤트·Blueprint 읽기 전용 배열은 호환 표시로 유지한다. 라운드 UI는 `FCombatPlanningRefreshState`로 계획·스킬·소유권·연결·AP/SAP·이동 범위·ASC 태그·타일 변화를 관찰하고, 동일 상태에서는 검증과 위젯 갱신을 생략한다. 타일 강조는 변경분만 반영한다. [사용자 확인](TODO.md#2-27-공통-검증과-실행-책임-분리)
...
`UEncounterResultWidget`은 Victory 아래에 골드 카드 3개·개인 잔액·수령 안내를 표시한다. 기본 금액은 각각 5~15G 독립 추첨이며 동일한 금액도 허용한다. 하나를 선택하면 서버가 저장한 금액을 본인 캐릭터에게 지급한다. 제출 중에는 카드를 비활성화하고 저장·복제 반영 후 선택한 카드에 `획득 완료`를 표시한다. 실패 시 오류와 재선택을 제공하며 본인 미수령 캐릭터가 여러 명이면 다음 캐릭터로 전환한다.

현재 Human 캐릭터가 모두 수령해야 Host의 Continue가 활성화된다. 다른 참가자는 수령·Host 진행 대기 안내를 받는다. 첫 승리는 보상 뒤 상점, 마지막 승리는 보상 뒤 Run 완료로 연결한다. 패배와 보상 도입 전 Result 저장에는 카드가 표시되지 않으며 기존 진행을 유지한다. 보상 선택지와 수령 기록은 이어하기에서 복원한다.

기존 `Text_Result`·`Button_Continue` 바인딩과 생성 `ContentBox`를 유지하고 네이티브 보상 영역을 추가하므로 WBP 재생성은 필요하지 않다. 공통 테마·DPI·인벤토리/설정 UtilityLayer 입력 차단을 따른다. 저장·소유권 기준은 [PROJECT_PLAN](PROJECT_PLAN.md#파티와-전투-규약), 실제 확인은 [TODO 2-24절](TODO.md#2-24-전투-승리-골드-보상)을 따른다.
