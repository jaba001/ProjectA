...
직업별 `BP_WarriorMenuPreview`·`BP_MageMenuPreview`·`BP_ArcherMenuPreview`·`BP_RogueMenuPreview`는 Kwang·Gideon·Sparrow·Countess 원본 메시를 참조한다. 각 뼈대로 리타깃한 `MM_Idle`을 SingleNode 모드·속도 1·반복·자동재생으로 연결하고 마법사에는 왼손 지팡이를 부착한다. `ConfigureProfessionAppearance.py`가 기존 카메라·앵커 배치와 NoCollision을 보존하며 작성한다. 이전 `BP_PartyMenuPreview`는 보존한다. 실제 반복 재생·위치·삭제/재진입은 [남은 확인](TODO.md#2-25-직업별-파라곤-외형)에서 확인한다.
...
표시 정보의 기준은 `UProfessionBase`와 네 C++ 자식 클래스다. 실사용 및 `Validation/T12` WBP의 기존 WidgetTree·배치·스타일·바인딩을 유지한다. MainMenu 프리뷰 맵은 네 직업 ID를 각 파라곤 프리뷰 Blueprint에 연결한다. 개발용 협동은 참가자마다 궁수 한 명을 생성한다.
