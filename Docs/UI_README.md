...
`URunEncounterWidget`은 기존 Gameplay RunLayer에서 상점 선택·구매 화면을 표시한다. `EncounterChoice`에서 Host가 상점 3곳 중 선택한다. `Shop`은 본인 캐릭터·개인 골드·현재/최대 HP, 스킬 4종과 1G HP 전체 회복 버튼을 표시한다. 최대 HP는 서버의 직업 설정에서 가져온다. 각 인간 참가자가 본인만 구매하며 스킬 중복·만피 회복·잔액 부족·사망·구매 처리 중 버튼은 비활성화한다. 구매 결과는 요청자에게 표시하고 나가기는 Host가 결정한다. CommonUI 메뉴 입력과 native 구성을 유지하며 WBP 재생성은 필요 없다. [남은 확인](TODO.md#2-19-비무장-시작과-스킬-상점)
...
전사의 `BP_WarriorMenuPreview`는 공통 Manny와 선택한 ROG 의상, 마법사의 `BP_MageMenuPreview`는 Stylized Dark Witch, 도적의 `BP_RogueMenuPreview`는 Assassin Skin1을 표시한다. 궁수는 Manny의 `BP_PartyMenuPreview`를 유지한다. 각 뼈대의 Idle을 반복 재생하며 마법사의 `Staff` 한 개는 왼손 `DEF-hand_L`에 연결한다.

캐릭터 **생성/수정**은 오른쪽 스크롤 편집창을 연다. 전사는 투구·상의/로브·바지·신발·장갑·어깨 장식·손목 보호대·망토를 선택하며 기본값은 선택 없음이다. 선택 즉시 같은 월드 프리뷰에 반영하고 회전 버튼으로 좌우 30도씩 돌린다. **저장**은 초안에 반영하고 **취소**는 이전 선택을 복원한다. 새 캐릭터 생성을 취소하면 빈 슬롯으로 복원하며 직업 변경 시 의상을 초기화한다. 기존 Designer WBP에도 C++ 공통 편집창으로 적용한다. [화면·저장 확인](TODO.md#2-30-rog-의상-커스터마이징) · [다른 직업 확인](TODO.md#2-29-마녀와-assassin-외형)
...
`UInventoryWidget`은 현재 참가자 캐릭터의 이름·직업·개인 골드·보유/장착 스킬을 표시한다. 사망 후에도 조회할 수 있고 구매·소유권·Run 저장을 변경하지 않는다. 명시 스킬 목록이 없는 이전 저장은 권위 Run의 실제 파티 목록으로만 해석하고, 협동 클라이언트는 확인 불가 안내를 표시한다. 장비 교체·아이템 소비는 구현 범위에 포함하지 않는다. 기존 DPI·테마를 사용하며 별도 WBP 생성은 필요 없다. 사용자 확인은 [TODO 2-20](TODO.md#2-20-인벤토리와-설정-단축키)을 따른다.

### 8-3 개인 골드 상시 표시

Gameplay 좌측 상단에 `보유 골드 10G` 형식의 작은 패널을 표시한다. 지도·상점·전투·결과에서 인벤토리와 같은 소유 캐릭터의 확정 잔액을 사용하며 구매·회복 저장 성공 또는 복제 뷰 도착 시 갱신한다. 실제 0G는 표시하고 캐릭터 미확정·Run 없음·협동 대기실에서는 숨긴다. 패널은 마우스 입력을 통과시키고 인벤토리·설정 레이어 아래에 배치한다. [사용자 확인](TODO.md#2-21-개인-골드-상시-표시)
