...
새 Run의 아이템상점은 원본 에셋 이름·가격·판매 완료 상태를 가진 아이템 5칸과 **리롤 1G**를 표시한다. 전체 295개에서 현재 진열 내 중복 없이 추첨하며 상품은 각 1G다. 구매하면 본인 보유 수량을 갱신하고 해당 슬롯을 비활성화한다. 리롤은 5칸을 새로 구성하고 이전 상품의 재등장을 허용한다. 잔액 부족·사망·요청 처리 중에는 구매·리롤을 막으며 서버의 저장 결과로 잔액과 진열을 갱신한다. 아이템상점 schema가 없는 이전 저장에는 새 Run 안내를 표시한다. [아이템상점 확인](TODO.md#2-31-아이템상점-시험)
...
`UInventoryWidget`은 상점과 같은 `UCharacterEquipmentPanel`·`UCharacterInventoryPanel`을 나란히 표시한다. 현재 참가자 캐릭터의 이름·직업·개인 골드·보유/장착 스킬과 아이템의 원본 에셋 이름·장착 상태를 조회한다. 같은 종류도 개별 사본으로 드래그하며 장착 사본과 가방 사본을 구분한다. 사망 후에도 조회할 수 있다. 명시 스킬 목록이 없는 이전 저장은 권위 Run의 실제 파티 목록으로만 해석하고, 협동 클라이언트는 확인 불가 안내를 표시한다.
