...
모든 상점 진입 시 `UCharacterEquipmentPanel`·상품 영역·`UCharacterInventoryPanel`을 좌우 3열로 표시한다. 기본 너비는 300·620·430 UI 단위이며 긴 상품명은 줄바꿈하고 각 목록은 스크롤한다. 상품 카드는 분류/스킬 아이콘·이름·가격·구매 버튼으로 구성한다. 인카운터 선택 화면에는 인카운터 선택 영역만 표시한다. 조회 캐릭터는 `GetInventoryCharacterId`로 가져와 사망 후에도 장비·인벤토리를 표시하고, 구매는 기존 생존 Human 검증을 따른다. 패널 내용과 장착 미구현 범위는 [8-2절](#8-2-gameplay-인벤토리와-설정-단축키), 사용자 확인은 [TODO 2-32](TODO.md#2-32-상점-장비와-인벤토리-ui)를 따른다.
...
`UInventoryWidget`은 상점과 같은 `UCharacterEquipmentPanel`·`UCharacterInventoryPanel`을 나란히 표시한다. 현재 참가자 캐릭터의 이름·직업·개인 골드·보유/장착 스킬과 구매한 아이템의 원본 에셋 이름·수량을 조회하며 동일 아이템은 전체 에셋 경로로 묶는다. 사망 후에도 조회할 수 있고 구매·소유권·Run 저장을 변경하지 않는다. 명시 스킬 목록이 없는 이전 저장은 권위 Run의 실제 파티 목록으로만 해석하고, 협동 클라이언트는 확인 불가 안내를 표시한다.

장비창은 확정한 무기 2칸·투구·장갑·신발·갑옷·목걸이 각 1칸·반지 2칸을 `미장착`으로 표시하고 `장착 기능 준비 중`을 안내한다. 현재 장비 데이터·장착 명령은 없으며 보유 아이템을 장착한 것으로 표시하지 않는다. 장착·능력치 적용·아이템 스킬 부여·메시 표시·소비는 미구현이다. 기존 CommonUI 입력·DPI·테마를 사용하며 별도 WBP 생성은 필요 없다. 사용자 확인은 [TODO 2-20](TODO.md#2-20-인벤토리와-설정-단축키)·[2-32](TODO.md#2-32-상점-장비와-인벤토리-ui)를 따른다.
