...
`URunEncounterWidget`은 기존 Gameplay RunLayer에서 상점 선택·구매 화면을 표시한다. `EncounterChoice`에서 Host가 상점 3곳 중 선택한다. 상점1·상점3은 기존 본인 캐릭터·개인 골드·현재/최대 HP, 스킬 4종과 1G HP 전체 회복 버튼을 유지한다. 최대 HP는 서버의 직업 설정에서 가져온다. 각 인간 참가자가 본인만 구매하며 스킬 중복·만피 회복·잔액 부족·사망·구매 처리 중 버튼은 비활성화한다. 구매 결과는 요청자에게 표시하고 나가기는 Host가 결정한다. CommonUI 메뉴 입력과 native 구성을 유지하며 WBP 재생성은 필요 없다. [스킬 상점 확인](TODO.md#2-19-비무장-시작과-스킬-상점)

새 Run의 상점2는 원본 에셋 이름·가격·판매 완료 상태를 가진 아이템 5칸과 **리롤 1G**를 표시한다. 전체 295개에서 현재 진열 내 중복 없이 추첨하며 상품은 각 1G다. 구매하면 본인 보유 수량을 갱신하고 해당 슬롯을 비활성화한다. 리롤은 5칸을 새로 구성하고 이전 상품의 재등장을 허용한다. 잔액 부족·사망·요청 처리 중에는 구매·리롤을 막으며 서버의 저장 결과로 잔액과 진열을 갱신한다. 아이템 상점 schema가 없는 이전 저장에는 새 Run 안내를 표시한다. [아이템 상점 확인](TODO.md#2-31-상점2-아이템-시험)
