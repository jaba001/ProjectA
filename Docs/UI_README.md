...
`UInventoryWidget`은 현재 참가자 캐릭터의 이름·직업·개인 골드·보유/장착 스킬과 구매한 아이템의 원본 에셋 이름·수량을 표시한다. 사망 후에도 조회할 수 있고 구매·소유권·Run 저장을 변경하지 않는다. 명시 스킬 목록이 없는 이전 저장은 권위 Run의 실제 파티 목록으로만 해석하고, 협동 클라이언트는 확인 불가 안내를 표시한다. 아이템 장착·스킬 부여·메시 표시·소비는 구현 범위에 포함하지 않는다. 기존 DPI·테마를 사용하며 별도 WBP 생성은 필요 없다. 사용자 확인은 [TODO 2-20](TODO.md#2-20-인벤토리와-설정-단축키)·[아이템 상점](TODO.md#2-31-아이템상점-시험)을 따른다.
