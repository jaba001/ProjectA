...
- 골드 차감과 스킬 획득은 함께 저장하며 실패 시 둘 다 확정하지 않는다. 기존 저장에 스킬·골드를 소급 지급하지 않는다.

10G·1G는 사용자 지정 시험값이다. 고정 판매 목록 외 추가 수입·보상·회복·재입고·확률 가중치·일반 아이템 거래는 별도 결정한다. 기존 콘텐츠 태그·GAS 실행 연결을 유지하며 고정 판매를 가중치 추첨 완료로 기록하지 않는다.
