...
현재 구현의 Grid·Turn·Movement·Targeting·Skill·GAS와 순차 전투 경로를 보존하면서 [8절](#8-라운드-계획과-시간차-자동-전투)의 새 전투를 별도 프로토타입으로 검증한다. 4×4 Grid를 출발점으로 사용하되 실제 공격 접근·복귀·실시간 피격에 필요한 점유·타기팅 계약은 후속 설계한다. Act·전투 라운드 수·목표 시간은 미정이며 계획·관전·실제 전투 시간을 구분해 평가한다. 목표 Run 흐름의 다음 Round와 전투 내부의 행동 라운드는 구분한다.
...
- 현재 순차 전투의 복구는 마지막 저장 완료된 확정 턴 경계를 사용한다. 새 시간차 전투의 복구 경계·저장 호환성은 미정이며 기존 지원을 자동 승계하지 않는다.
...
현재 실행 파일은 기존 개별 유닛 순차 턴이다. 본 절의 시간차 전투·속도 데이터·실시간 접근 및 피격 전환은 미구현이다. 코드·Blueprint·Asset·Config·저장 버전을 이번 문서 작업에서 변경하지 않는다. 세부 계약의 미결정 사항은 [TODO 8절](TODO.md#8-시간차-자동-전투-후속-결정)에 둔다.
...
플레이어는 계획 단계에서만 행동을 선택한다. 해결 도중 재선택·단계 사이 재확정을 요구하지 않는다. 초안과 최종 명령을 구분하며, 인간은 원래 자기 캐릭터의 행동만 편집한다. 초기 검증은 강제 선택 제한시간 없이 준비 완료 방식으로 설계한다. 준비 취소·팀원 수정·미응답의 상세 처리는 미정이다.
...
기준 최고 속도의 참여 집합·속도 채집 시점·라운드 중 변경 처리는 후속 명세로 확정한다. 제안은 양 진영의 라운드 참여 생존 유닛을 기준으로 시각표를 한 번 고정하고, 이후 사망·속도 변화로 다른 유닛의 예약 시각을 당기거나 늦추지 않는 방식이다. 제안 자체를 구현 완료 또는 확정 정책으로 취급하지 않는다.

속도에 따른 이동속도·시전속도·스킬별 투사체 속도 보정은 후속 상세 기획이다. 프로토타입용 고정 이동·시전·투사체 수치가 필요하며, 시작 지연 공식을 해당 수치에 자동 적용하지 않는다. AP·보조 AP는 현재 코드에서 유지하고 한 행동 예약의 비용·초기화 정책은 별도 결정한다.
...
유닛 간 이동 충돌을 없애는 것과 투사체·근접 공격의 피격 검출은 별도 계약이다. 피격 검출까지 비활성화하는 의미가 아니다. 타일 점유와 실제 좌표의 역할, 이동 중 중첩·목적지 및 복귀 칸 경합, 이동 대상 추적·접근 거리·복귀 실패 처리는 아직 미정이다. 사망한 유닛의 복귀를 강제하지 않으며 사망 시 행동 취소 규칙을 적용한다.

현재 코드의 공격 접근 연출 중 원래 타일 점유를 유지하는 동작을 새 피격 위치의 근거로 그대로 사용하지 않는다. 전열 보호를 실제 접근 중에도 적용할지, 어느 위치를 기준으로 판단할지는 별도 결정한다.
...
같은 시각의 타격 우선순위·상호 치명타, 투사체 유도·관통·가로막기·아군 피격, 다단 타격 중 사망·중복 적중, 상태효과의 즉시 취소 여부는 미정이다. 이전 제안의 다음 라운드 상태효과 적용이나 무승부 처리도 자동 확정하지 않는다.

### 8-6 라운드 종료와 적 계획

모든 유닛의 예약 행동이 완료 또는 취소되고, 남은 유효 투사체와 지연 피해가 정리된 상태에서 라운드를 종료한다. 일반 공격의 복귀도 행동 완료에 포함한다. 아직 유효한 공격이 남았다면 최초 사망이나 마지막 발사만으로 결과를 확정하지 않는다. 동시 전멸의 최종 결과·보상 정책은 미정이다.

투사체 최대 수명·지연 피해 종료 기준·이동 실패 정리·취소 완료 조건은 무한 대기 방지를 위해 구현 전에 명세한다. 이는 플레이어에게 강제 선택 제한시간을 도입하는 의미가 아니다.

적은 각 라운드의 시작 상태를 기준으로 인간 계획 입력 전에 행동을 결정·고정한다. 인간의 비공개 초안을 보고 재판단하지 않는다. 고정 명령의 실행에 필요한 대상 추적·목표 사망 대응은 스킬 정책으로 정의하며 새 전략 선택과 구분한다. 초기 전체 의도 공개는 검증 제안으로 유지하고 공개 수준의 최종 채택은 별도 결정한다. Snapshot AI 방어 팀과 의도를 보는 인간 공격 팀의 조건은 비대칭이므로 경쟁 밸런스·보상을 별도로 검토한다.

### 8-7 구현 경계와 최소 실험

기존 순차 전투와 분리된 2 대 2를 시작점으로 제안한다. 라운드·유닛·명령은 배열과 안정적인 식별자로 표현하고 2인에 하드코딩하지 않는다. 최대 4인·Human/AI 혼합으로 확장할 때 원래 소유권을 유지하며 필수 준비 대상과 고정 AI 명령을 구분한다.

최소 검증은 유닛 접근형 근접, 타일 접근형 스킬, 발사 후 독립적으로 남는 투사체, 속도순 지원을 대상으로 한다. 스킬별 이동·시전·비행 수치는 고정 시험값을 명시한다. 실제 두 사람이 함께 계획하는 검증은 로컬 단독 규칙 검증과 분리한다.

현재 TurnManager의 활성 유닛 검사·AI 연속 판단·즉시 전투 종료·능력 종료 시 공격 Actor 정리·타일 점유가 새 규칙과 충돌하는지 후속 구현에서 확인한다. 기존 진입 검사를 단순 해제하지 않고 계획 수집 → 확정 검증 → 시간 기반 실행 → 결과 표현의 책임을 구분한다. 새 Manager 또는 스키마의 개수는 확정하지 않는다.

전체 네트워크 재작성·신규 매칭·신규 저장 시스템·부분 정보 공개 시스템·반격 연쇄·복잡한 상태효과는 초기 필수 범위에서 제외한다. 기존 Run·AP·저장 형식과 정상 전투 경로는 이번 기획으로 즉시 교체하지 않는다. 네트워크·복구 경계는 [MULTIPLAYER 12절](MULTIPLAYER.md#12-시간차-자동-전투의-확장-경계), 검증 계획은 [TEST_REPORT 12절](TEST_REPORT.md#12-시간차-자동-전투-기획-검토)를 따른다.
