...
이전 순차 Combat 저장의 로드·관리 재개·새 체크포인트 저장은 명시적으로 거절한다. 파일과 원래 소유권을 덮어쓰지 않는다. 새 전투 중간 복구는 제공하지 않으며 마지막 전투 외 확정 기록에서 Encounter를 다시 시작할 수 있다. 네트워크·저장 계약은 [MULTIPLAYER 12절](MULTIPLAYER.md#12-시간차-자동-전투의-확장-경계), 검증 절차는 [남은 확인](TODO.md#1-사용자-작동-확인)을 따른다.
