...
- 2026-09-25 아이템상점 시험은 [무기 CSV](WEAPON_ASSETS.csv)의 방패·탄환·화살·기타를 포함한 전체 295개에서 중복 없이 5개를 제시한다. 원본 에셋 이름과 `가격(G)`을 사용하고 현재 모든 상품·리롤 비용은 1G다. 구매는 개인 Run 아이템 수량에 반영하고 슬롯을 판매 완료로 표시한다. 리롤에서 이전 상품의 재등장을 허용하며 장착은 [2-2절](#2-2-장비-슬롯과-장비-부여-스킬)의 별도 드래그 명령을 사용한다.
