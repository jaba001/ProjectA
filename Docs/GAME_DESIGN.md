...
2026-09-22 사용자 확정. 아래 슬롯과 장비에 스킬을 부여하는 방향을 기획 문서에만 반영한다.
...
직업 스킬·별도 습득 스킬과 장비 스킬의 관계, 장착·교체 가능 시점·비용, 방패 사용 여부·점유 규칙은 후속 결정한다. 현재 비무장 시작·스킬 상점 구매와 아이템상점의 아이템 보유·리롤 시험은 유지한다. 아이템 구매는 장착·스킬 부여·메시 표시를 포함하지 않으며 장비 시스템 구현 완료로 간주하지 않는다.
...
- 2026-09-25 아이템상점 시험은 [무기 CSV](WEAPON_ASSETS.csv)의 방패·탄환·화살·기타를 포함한 전체 295개에서 중복 없이 5개를 제시한다. 원본 에셋 이름과 `가격(G)`을 사용하고 현재 모든 상품·리롤 비용은 1G다. 구매는 개인 Run 아이템 수량에 반영하고 슬롯을 판매 완료로 표시한다. 리롤에서 이전 상품의 재등장을 허용한다. 장착·스킬 부여·메시 표시는 후속 작업이다.
