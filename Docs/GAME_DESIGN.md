...
기준일: 2026-09-22. 목표 기획·확정 방향과 구분된 콘텐츠 제안을 정의한다. 구현 상태는 [PROJECT_PLAN](PROJECT_PLAN.md), 미결정 사항은 [TODO](TODO.md), 세부 네트워크 정책은 [MULTIPLAYER](MULTIPLAYER.md)를 따른다. 아이템 클론 기획은 [7절](#7-아이템-클론-기획), 라운드 계획·시간차 자동 전투의 확정 방향은 [8절](#8-라운드-계획과-시간차-자동-전투)에 둔다.
...
현재 구현은 동일 Gameplay의 두 전투 노드를 시간차 라운드 전투로 진행하며 패배 시 Run을 종료한다. 승리마다 5~15G 보상 3개 중 하나를 수령한다. 새 Run의 첫 승리 보상 뒤에는 상점1·상점2·상점3을 고정 제시하고 하나를 선택한다. 개인 골드로 본인 스킬을 구매한 뒤 두 번째 전투로 연결한다. 마지막 승리도 보상 수령 뒤 Run을 종료한다.
...
직업 스킬·별도 습득 스킬과 장비 스킬의 관계, 장착·교체 가능 시점·비용, 방패 사용 여부·점유 규칙은 후속 결정한다. 현재 비무장 시작·스킬 상점 구매 구현은 유지하며 장비 시스템 구현 완료로 간주하지 않는다.
...
- 상점 3곳 모두 검·원거리·AOE·휩쓸기를 각 1G에 판매한다. 직업 제한 없이 같은 스킬을 캐릭터별 한 번 구매하고 Run 동안 유지한다.
- HP 전체 회복을 1G에 판매한다. 생존한 본인 캐릭터의 HP를 구매 즉시 최대치까지 회복하며 만피·사망 상태에서는 구매할 수 없다.
- 협동에서는 원래 본인 소유의 생존 인간 조작 캐릭터만 구매한다. Host의 타인 구매와 AI 전환 캐릭터 구매를 허용하지 않는다. 상점 선택·퇴장은 현재 Host가 결정한다.
- 골드 차감과 스킬 획득 또는 HP 회복은 함께 저장하며 실패 시 둘 다 확정하지 않는다. 기존 스킬 상점 저장에는 회복 상품만 추가하고 골드·스킬을 보존한다. 스킬 상점 도입 전 저장에는 상품·골드를 소급 지급하지 않는다.
...
추가 보상 종류·재입고·확률 가중치·일반 아이템 거래는 별도 결정한다. 골드 수치 추첨은 임시 시험 규칙이며 아이템/스킬 후보의 태그 기반 가중치 생성 완료를 의미하지 않는다. 기존 콘텐츠 태그·GAS 실행 연결을 유지한다.
...
현재 시작값이며 직업별 최종 밸런스·성장률·무기 보정을 확정한 것은 아니다. 외형은 전사 Kwang·궁수 Sparrow·마법사 Gideon·도적 Countess이며 마법사는 `Mage Staff - Free Weapons`의 지팡이를 든다. AP는 유지한다. 2026-09-22부터 새 Run의 시작 스킬은 모두 비무장 공격 하나이며 추가 스킬은 상점에서 구매한다. 힘·민첩·지능의 피해·명중 보정은 별도다. [현재 스킬 구성](#8-7-기본-전투-전환과-스킬-데이터)
...
Paragon Kwang의 `PrimaryAttack_A_Slow` 1.2초와 `PrimaryAttack_A_Slow_Recovery` 0.933333초를 네 파라곤 직업과 기존 적 뼈대에 맞춰 사용한다. 공격 끝부분과 포즈가 겹치는 복귀 클립의 첫 0.2초를 제외한 총 1.933333초·블렌드 인 0.08초/아웃 0.12초를 유지한다. 검 부착은 Roll 180도·Yaw 0도이며 기존 Boss Swing 사본·Blueprint의 4/5개 기본 스킬·Snapshot 입력의 저장된 스킬을 보존한다. 기본 적은 검 하나와 검 공격만 사용한다. 검 방향·실제 접촉은 [사용자 확인](TODO.md#2-25-직업별-파라곤-외형) 대기다.
