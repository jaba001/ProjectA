...
현재 구현은 동일 Gameplay의 두 전투 노드를 시간차 라운드 전투로 진행하며 패배 시 Run을 종료한다. 새 Run의 첫 승리 뒤에는 상점1·상점2·상점3을 고정 제시하고 하나를 선택한다. 개인 골드로 본인 스킬을 구매한 뒤 두 번째 전투로 연결한다. 마지막 승리 뒤에는 기존 Run 종료를 유지한다.
...
10G·1G는 사용자 지정 시험값이다. 고정 판매 목록 외 추가 수입·보상·재입고·확률 가중치·일반 아이템 거래는 별도 결정한다. 기존 콘텐츠 태그·GAS 실행 연결을 유지하며 고정 판매를 가중치 추첨 완료로 기록하지 않는다.
