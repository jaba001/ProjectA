...
현재 시작값이며 직업별 최종 밸런스·성장률·무기 보정을 확정한 것은 아니다. 전사는 GKnight 외형, 다른 직업은 기존 외형·AP를 유지한다. 2026-09-22부터 새 Run의 시작 스킬은 모두 비무장 공격 하나이며 추가 스킬은 상점에서 구매한다. 힘·민첩·지능의 피해·명중 보정은 별도다. [현재 스킬 구성](#8-7-기본-전투-전환과-스킬-데이터)
...
Paragon Kwang의 `PrimaryAttack_A_Slow` 1.2초와 `PrimaryAttack_A_Slow_Recovery` 0.933333초는 GKnight 전사와 기존 적 작업 사본에 IK 리타깃한다. 공격 끝부분과 포즈가 겹치는 복귀 클립의 첫 0.2초를 제외한 총 1.933333초·블렌드 인 0.08초/아웃 0.12초를 유지한다. 잘못 저장된 Yaw 180도를 Roll 180도·Yaw 0도로 바로잡고 손잡이가 손에 맞도록 부착 위치를 보정한다. 기존 Boss Swing 사본·전사 기존 4스킬·Snapshot 입력의 저장된 스킬을 보존하며 기본 적은 검 하나와 검 공격만 사용한다. 검 방향·실제 접촉은 [사용자 확인](TODO.md#2-15-전사와-검-공격-콘텐츠) 대기다.
