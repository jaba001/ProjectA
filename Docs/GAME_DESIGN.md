...
현재 시작값이며 직업별 최종 밸런스·성장률·스킬·무기 보정을 확정한 것은 아니다. 공통 전투 Blueprint·외형·기존 AP와 장착 스킬을 유지하며 직업별 공격을 임의로 작성하지 않는다. 힘·민첩·지능의 피해·명중 보정은 별도다.
...
근접 접근·복귀의 속도 보정은 [8-4절](#8-4-공격-접근과-복귀)을 따른다. SAP·비근접 스킬 이동·시전·투사체 속도에는 이번 민첩 보정을 적용하지 않는다. AP·보조 AP는 유지한다. 초기 구현은 라운드 계획 시작에 초기화하고 전체 명령 잠금 시 한 번 차감한다. 한 주요 행동만 예약하며 이후 불발·사망에 비용을 환불하지 않는다. 직업·아이템의 세부 자원 밸런스는 후속 검토다.
...
기본 `MoveSpeed=700`에서 속도 0·5·10·20의 접근·복귀는 각각 175·262.5·350·525cm/s다. Planning에서 고정한 Speed를 사용하며 실행 중 민첩 변경은 다음 라운드에 반영한다. 음수·비유한 Speed는 0으로 취급한다. SAP 이동·비근접 스킬·시전·투사체 속도는 변경하지 않는다.
