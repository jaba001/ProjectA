...
구현 전에 선택할 정책과 영향은 [TODO의 아이템 후속 결정](TODO.md#3-2-아이템과-직업)에 모은다. 기획 검토·추후 작동 확인 절차는 [TEST_REPORT 11절](TEST_REPORT.md#11-아이템-클론-기획-검토)을 따른다.
...
ProjectA에는 직업 역할·필수 보급과 장비 슬롯을 분리한 상점·약초 부가 효과 모델을 **채택 미확정 제안**으로 보충했다. 기존 시간차 전투와 상태이상 규칙을 유지하며 슬롯 판정·회복 모델·최종 슬롯·개인/공동 재화는 [TODO 3-2절](TODO.md#3-2-아이템과-직업), 경쟁 Snapshot 범위는 [TODO 4-2절](TODO.md#4-2-서비스-구현과-검증)의 사용자 결정 대상으로 둔다. 확인 절차와 미실행 항목은 [TEST_REPORT 11절](TEST_REPORT.md#11-아이템-클론-기획-검토)에 기록한다.
...
현재 기본 Combat를 본 절의 시간차 전투로 교체했다. 기존 순차 턴·연속 AI 행동·End Turn 입력 실행은 제거했으며 기존 Blueprint의 클래스·프로퍼티 참조용 외형만 유지한다. 초기 C++ 구현과 작동 검증 완료는 구분한다. 준비 취소·복귀 칸·동시 타격의 임시 구현 정책은 아래와 [TODO 3-1절](TODO.md#3-1-전투-규칙)에 명시하며 사용자 확정 정책으로 취급하지 않는다.
...
초기 AI는 기존 스킬 순서와 가까운 적 기준을 유지한다. 장착된 복귀형 Tile 공격은 적의 라운드 시작 HomeCoord를 목표와 필요한 접근 좌표로 구성한다. 공통 시험 GroundStrike·MoveShot·Guard를 새 전술 후보로 추가하지 않으며 지원·잔류 이동 전술은 [디자인 보류 항목](TODO.md#5-디자인-확정-후-구현할-일)으로 둔다. 실행 가능한 기존 후보가 없으면 Wait를 사용한다.
