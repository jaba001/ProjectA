...
| 스킬·아이템 | 실제 장착 스킬의 라운드 변환·SUP 위치 이동. 공통 시험 행동과 자동 추가 스킬 부여 제거 | 획득·장착·소모·강화 규칙 |
...
2026-09-16 사용자 요청으로 **민첩 1당 전투 속도 1**을 적용한다. 네 직업의 시작 속도는 10이며 일반 테스트 적의 힘·민첩·지능은 각각 5, 속도는 5다. 일반 적의 기존 HP 150·AP 2는 유지한다. Snapshot 적은 저장된 능력치를 사용하며 일반 적의 5로 덮어쓰지 않는다. 전투 속도는 [8-3절](#8-3-행동-시작-시간)의 시작 지연에만 적용한다.
...
선택은 저장·이어하기·다음 전투에서 유지한다. 직접 조작 캐릭터가 사망해도 생존 동료가 있으면 AI 전투를 계속하고 전체 아군의 생존 상태로 승패를 판정한다. 관리 Run·협동·관리 싱글 전환의 기존 인간 참가자/소유권 정책은 유지한다. 기존 저장의 구체적 호환은 [PROJECT_PLAN](PROJECT_PLAN.md#저장과-멀티플레이-연결-경계), 확인 절차는 [TEST_REPORT 25절](TEST_REPORT.md#25-싱글플레이-직접-조작-캐릭터-선택)을 따른다.
...
ProjectA에는 직업 역할·필수 보급과 장비 슬롯을 분리한 상점·약초 부가 효과 모델을 **채택 미확정 제안**으로 보충했다. 공격 적중은 [8-5절의 충돌 판정](#8-5-발동과-피격)으로 확정했다. 기존 시간차 전투와 상태이상 규칙을 유지하며 원작 완전 성공 부가 효과의 변환·회복 모델·최종 장비 슬롯·개인/공동 재화는 [TODO 3-2절](TODO.md#3-2-아이템과-직업), 경쟁 Snapshot 범위는 [TODO 4-2절](TODO.md#4-2-서비스-구현과-검증)의 사용자 결정 대상으로 둔다. 확인 절차와 미실행 항목은 [TEST_REPORT 11절](TEST_REPORT.md#11-아이템-클론-기획-검토)에 기록한다.
...
2026-09-16 사용자 요청에 따라 전장의 적을 클릭한 뒤 하단의 실제 장착 스킬 버튼으로 행동을 적용한다. 준비 완료로 기존 라운드를 잠그며 해결 중 재선택은 막는다. 적용 전 자원·타일·대상을 검사하고 원래 자신이 조작하는 모든 생존 유닛의 유효한 계획을 요구한다. 선택 대상 변경만으로 기존 명령을 바꾸지 않으며, 하단에 적용된 행동과 대상을 따로 표시한다. 소유권·수정 번호·최종 예약 충돌과 팀 준비 초기화의 기존 정책을 유지한다.

SUP 위치 이동은 계획 중 별도의 보조 행동으로 복구한다. `이동 · SUP 1`을 누른 뒤 이동 범위 안의 아군 빈칸을 선택한다. AP를 요구하거나 소비하지 않으며 기존 8방향 빈칸 BFS와 MoveRange를 사용한다. 승인 즉시 SUP 1을 소비하고 실제로 이동하며, 이동 중에는 추가 이동·계획·준비 요청을 막는다. 도착 후 새 칸을 HomeCoord로 사용하여 이후 공격이 그 자리로 복귀한다. 이동 실패·중단 시 생존자는 출발 위치로 복원하고 SUP를 환불하지 않는다. 주요 행동 하나를 예약하는 규칙과 별개다.
...
현재 구현은 0.01초 서버 진행 단위에서 유닛·공격 사건을 안정적인 서버 순서로 처리하고 피해·사망을 즉시 반영한다. 같은 예정 시각의 상호 공격을 모두 보장하지 않는 임시 정책이며 사용자 확인이 필요하다. 아군 피해·투사체 관통·다단 타격·복잡한 상태효과는 초기 범위에서 제외한다. 유도·지정 대상 한정 여부와 발동 전 목표 상실 처리는 스킬 프로필로 설정한다. 최신 충돌 판정의 사용자 작동 확인은 미실행이다.
...
실전 스킬 목록은 검증된 실제 장착 DA만 사용한다. 공통 시험 `Strike`·`Arrow`·`Guard`·`Wait`·`MoveShot`·`GroundStrike` 주입과 무장착 대체 공격을 제거했다. Encounter 진입 시 시험용 스킬 풀을 자동 추첨·장착하지 않는다. 현재 사용자 제작 공격은 기본 공격 한 개이며 이름·ID·몽타주를 유지한다. 시험 정의는 명시적으로 장착하는 C++ 회귀 fixture에서만 사용한다. 기존 Guard 등의 해석 코드가 남아 있는 사실은 해당 콘텐츠 제작 완료를 뜻하지 않는다.
