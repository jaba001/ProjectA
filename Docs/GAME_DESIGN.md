...
기준일: 2026-09-25. 목표 기획·확정 방향과 구분된 콘텐츠 제안을 정의한다. 구현 상태는 [PROJECT_PLAN](PROJECT_PLAN.md), 미결정 사항은 [TODO](TODO.md), 세부 네트워크 정책은 [MULTIPLAYER](MULTIPLAYER.md)를 따른다. 아이템 클론 기획은 [7절](#7-아이템-클론-기획), 라운드 계획·시간차 자동 전투의 확정 방향은 [8절](#8-라운드-계획과-시간차-자동-전투)에 둔다.
...
2026-09-22 사용자 확정: 목표 플레이 시간은 사전에 고정하지 않는다. 아래 진행 묶음을 10회 반복하여 Async PvP를 총 10회 치른다.

```text
파티 생성 → Run 시작
→ [인카운터 1 → 인카운터 2 → 인카운터 3 → 몬스터 PvE
   → 인카운터 4 → 인카운터 5 → 인카운터 6 → Async PvP] × 10
→ Run 결과
```

정상 완주 기준으로 인카운터 60회·몬스터 PvE 10회·Async PvP 10회다. PvP 10회는 승리 횟수가 아닌 전투 횟수다. 인카운터 3회는 세 차례 연속 진행을 뜻하며, 각 회차에는 기존의 **후보 3개 제시 → 하나 선택 → 선택한 인카운터 진행** 규칙을 적용한다. 진행 묶음과 전투 내부 행동 라운드는 구분한다. 패배·사망 처리는 기존 확정 정책을 따른다.

위 구성은 목표 기획이며 아직 구현하지 않았다. 단계별 인카운터 풀·몬스터 편성·난이도·성장/보상 수치와 PvP 상대 선정·MMR 계산 방식은 별도 결정한다.

현재 구현은 동일 Gameplay의 두 전투 노드를 시간차 라운드 전투로 진행하며 패배 시 Run을 종료한다. 승리마다 5~15G 보상 3개 중 하나를 수령한다. 새 Run의 첫 승리 보상 뒤에는 스킬상점·아이템상점·상점3 인카운터를 고정 제시하고 하나를 선택한다. 스킬상점·상점3은 개인 골드로 본인 스킬·회복을 구매하고 아이템상점은 아이템 구매·리롤을 시험한 뒤 두 번째 전투로 연결한다. 마지막 승리도 보상 수령 뒤 Run을 종료한다.

| 영역 | 현재 구현 | 확장 목표 |
|---|---|---|
| 성장 | 이름·직업·HP·개인 골드·습득 스킬·노드 진행 보존 | Run 단위 장비·보상 누적 확장 |
...
### 2-1 성장·경제·보상과 Run 종료

2026-09-22 사용자 확정. 기획 문서에만 반영하며 구현은 진행하지 않는다.

- Run 내 성장·재화·스킬·아이템은 **몬스터 PvE 보상과 인카운터**에서 획득한다. PvP에는 별도의 성장·재화·아이템 획득 보상을 두지 않는다. 구체적인 지급 종류·수량·가격·확률은 별도 결정한다.
- **회복·부활은 추후 인카운터 콘텐츠로 추가한다.** 등장 조건·적용 대상·회복량·부활 상태·비용은 후속 기획이다.
- **Run 종료 시 MMR만 반영한다.** Run에서 얻은 성장·능력치 보정·골드·장비·습득 스킬·소모품은 다음 Run에 이월하지 않으며 계정 영구 성장 보상도 두지 않는다.
- MMR 계산식·개별 PvP 결과의 합산 방식·반영 대상 확정 기준·이탈 처리는 [멀티플레이 계약](MULTIPLAYER.md)에 따라 후속 결정한다.

이월 금지는 다음 Run의 플레이 자산에 대한 규칙이다. 진행 중 저장·이어하기와 상대 Snapshot·결과 검증용 기록의 보존은 별도 계약을 따른다. 현재 시험용 5~15G 승리 보상과 상점의 HP 전체 회복 1G 구현은 유지하며 목표 기획의 구현 완료로 간주하지 않는다.

### 2-2 장비 슬롯과 장비 부여 스킬

2026-09-22 슬롯과 장비 부여 스킬 방향 확정. 2026-09-25 시작 장비와 상점 드래그 장착을 추가하며 장비 부여 스킬·능력치는 후속 구현으로 구분한다.

| 장비 부위 | 슬롯 수 | 장착 규칙 |
|---|---|---|
| 무기 | 2 | 양손 무기 1개가 두 손을 점유하거나 한손 무기를 최대 2개 장착 |
| 투구 | 1 | 투구 1개 |
| 장갑 | 1 | 장갑 1개. 무기 슬롯과 구분 |
| 신발 | 1 | 신발 1개 |
| 갑옷 | 1 | 갑옷 1개 |
| 목걸이 | 1 | 목걸이 1개 |
| 반지 | 2 | 반지 최대 2개. 동일 반지 중복 장착 여부는 미정 |

논리 슬롯은 총 9칸이다. 모든 부위를 채우면 양손 무기 구성은 장비 8개, 한손 무기 2개 구성은 장비 9개가 된다.

장비에 스킬을 부여하여 빌드를 구성하는 방향을 채택한다. 장비별 스킬 종류·개수·부여 방식, 장착/해제 시 부여·회수와 같은 스킬의 중복 처리는 별도 결정한다. 모든 장비에 스킬을 반드시 부여하거나 스킬 수를 하나로 제한하는 규칙은 아직 정하지 않았다.

새 Run의 시작 장비는 전사 한손검+방패·궁수 활·마법사 스태프·도적 단검이다. 시작 장비는 장착 상태로 지급하고 `I` 인벤토리에서 조회한다. 활과 사용자 확정 시작 스태프는 양손을 점유한다. 원본 에셋이 없으면 해당 슬롯을 비우며 현재 선택한 5개 원본은 모두 존재한다.

장비 변경은 본인 생존 Human의 **상점 페이즈 드래그 앤 드롭**으로 처리한다. 인벤토리에서 장비창으로 장착하고 가방으로 돌려 해제하며 호환되는 한손 슬롯끼리는 교체한다. 겹치는 양손 장비와 교체 불가능한 기존 장비는 가방에 남긴다. 장착 변경은 별도 골드를 차감하지 않고 저장 성공 후 반영한다. 시작 장비를 이전 저장에 소급 지급하지 않으며 기존 보유 아이템은 개별 장착할 수 있다.

현재 장착 프로필은 단검·방패·활과 명시적인 한손검·양손검 및 시작 스태프의 49개 원본을 지원한다. 나머지 CSV 246개는 소유·구매를 유지하고 한손/양손·부위·부착 설정을 후속 결정한다. 직업 스킬·별도 습득 스킬과 장비 스킬의 관계, 능력치 보정·아이템 스킬·경쟁 Snapshot 연결은 미구현이다. [구현·확인](TODO.md#2-33-시작-장비와-상점-드래그-장착)
...
사용자 지시에 따라 기존 개별 유닛 순차 전투를 시간차 라운드 전투로 교체한다. 별도 순차 모드나 기존 AI 실행 경로는 유지하지 않는다. 기존 4×4 Grid와 Run·직업·소유권을 연결하며 Blueprint 참조용 외형만 보존한다. Run의 진행 순서·횟수는 [2절](#2-목표-run과-현재-구현)을 따르며 목표 플레이 시간은 사전에 고정하지 않는다. Act 구분과 전투 내부 행동 라운드 수는 별도 미정이다.
...
- AI 전환 후 현재 인간 참가자에게만 Run 종료 시 MMR을 반영한다. 계산식·결과 합산·반영 대상 확정 기준·이탈 악용 대응은 미정이다.
...
각 인카운터의 확정 선택 규칙은 **후보 3개 제시 → 하나 선택 → 선택한 인카운터 진행**이다. 목표 Run은 [2절](#2-목표-run과-현재-구현)의 순서에 따라 PvE 전후에 각각 인카운터를 3회 진행한다. 기존 상점1은 **스킬상점**, 상점2는 **아이템상점** 인카운터로 정의하며 상점3은 기존 시험용 인카운터로 유지한다. 현재 두 전투 사이에 한 번 방문하는 상점 인카운터의 규칙은 다음과 같다.

- 새 Run의 아군 네 직업은 비무장 공격만 소유한다. 기본 적의 검 공격과 기존 Snapshot 스킬은 유지한다.
- 직접 조작 캐릭터별 개인 10G로 시작한다. AI 동료에게 골드를 지급하거나 구매 스킬을 배분하지 않는다.
- 스킬상점·상점3은 기존 검·원거리·AOE·휩쓸기를 각 1G에 판매한다. 직업 제한 없이 같은 스킬을 캐릭터별 한 번 구매하고 Run 동안 유지한다.
- 스킬상점·상점3은 HP 전체 회복을 1G에 판매한다. 생존한 본인 캐릭터의 HP를 구매 즉시 최대치까지 회복하며 만피·사망 상태에서는 구매할 수 없다.
- 2026-09-25 아이템상점 시험은 [무기 CSV](WEAPON_ASSETS.csv)의 방패·탄환·화살·기타를 포함한 전체 295개에서 중복 없이 5개를 제시한다. 새 Run은 `게임 내 이름`과 `가격(G)`을 사용하고 기존 저장은 저장 당시 이름을 유지한다. 현재 모든 상품·리롤 비용은 1G다. 구매는 개인 Run 아이템 수량에 반영하고 슬롯을 판매 완료로 표시한다. 리롤에서 이전 상품의 재등장을 허용하며 장착은 [2-2절](#2-2-장비-슬롯과-장비-부여-스킬)의 별도 드래그 명령을 사용한다.
- 협동에서는 원래 본인 소유의 생존 인간 조작 캐릭터만 구매한다. Host의 타인 구매와 AI 전환 캐릭터 구매를 허용하지 않는다. 상점 선택·퇴장은 현재 Host가 결정한다.
- 골드 차감과 스킬 획득·HP 회복 또는 아이템 구매·진열 변경은 함께 저장하며 실패 시 확정하지 않는다. 기존 저장의 골드·스킬·아이템상점 상태는 보존한다. 아이템상점 데이터가 없는 이전 저장(schema 0)에는 새 Run 안내를 표시하며 상품·골드를 소급 지급하지 않는다.

10G·1G는 사용자 지정 시험값이다. 2026-09-22 요청의 임시 승리 보상은 5~15G 무작위 선택지 3개 중 하나다. 각 값은 독립 추첨하여 동일 금액이 포함될 수 있다. 선택지는 승리 확정 시 한 번 생성·저장하고 수령 골드와 선택 기록을 함께 저장한다. 파티 승리 시 직접 조작 캐릭터는 생존 여부와 관계없이 개인 보상을 고르며 AI 동료는 받지 않는다. 협동은 현재 인간 참가자마다 본인 보상을 고른 뒤 Host가 계속한다. 마지막 전투도 수령 후 Run을 종료한다. 저장 실패·재개로 중복 지급하거나 공개된 선택지를 다시 생성하지 않는다. 기존 상점 저장은 다음 승리부터 적용하며 과거 결과와 상점 이전 저장에는 소급하지 않는다.

추가 보상 종류·최종 재입고·확률 가중치·플레이어 간 아이템 거래는 별도 결정한다. 골드 수치와 아이템상점 상품 추첨은 임시 시험 규칙이며 콘텐츠별 태그 가중치 설계의 완료를 의미하지 않는다. 기존 콘텐츠 태그·GAS 실행 연결을 유지한다.
...
현재 시작값이며 직업별 최종 밸런스·성장률·무기 보정을 확정한 것은 아니다. 파라곤 캐릭터 적용은 롤백했으며 전투 외형은 전사 GKnight·나머지 Manny로 복구했다. 새 외형 에셋은 미선정이다. AP는 유지한다. 2026-09-22부터 새 Run의 시작 스킬은 모두 비무장 공격 하나이며 추가 스킬은 상점에서 구매한다. 힘·민첩·지능의 피해·명중 보정은 별도다. [현재 스킬 구성](#8-7-기본-전투-전환과-스킬-데이터)
...
| 장비 역할 | 몸·머리·발, 방패, 목걸이·장신구로 방어·능력치·면역·스킬 보완 | 손 장비 및 재질 보정 등 장비 조합 확대 | 원작 슬롯과 ProjectA 슬롯을 구분. ProjectA는 [2-2절](#2-2-장비-슬롯과-장비-부여-스킬)의 9칸 구성 적용 |
...
| A04 치명 장갑 | FTK2 Light Gloves | 설치 정의 CRT +10·저항 +1 | 장갑 슬롯을 통한 공격 보완 | 장갑 슬롯 적용·CRT 적용 단위·확률 상한 |
...
| R03 물리 방어 반지 | FTK2 Armored Ring | HP +3·방어 +2 | 특정 피해 유형 대응 | 반지 슬롯 적용·동일 반지 중복 장착 규칙 |
| R04 마법 저항 반지 | FTK2 Magic Ring | HP +3·저항 +2 | R03과 상대 구성별 선택 | 반지 슬롯 적용·마법 피해 채택 |
...
| P02 영구 공격 성장 사탕 | FTK2 Black Candy | 소비자 물리 피해 영구 +1 | 장착 슬롯 밖의 성장 | 채택 시 해당 Run 내에서만 지속·누적 한도 미정. 계정 이월 없음 |
...
ProjectA의 획득 경로·회복/부활 도입·Run 종료 정책은 [2-1절](#2-1-성장경제보상과-run-종료)을 따른다. 아래 성장 후보를 채택하더라도 효과는 해당 Run에 한정하며 계정 영구 성장으로 이월하지 않는다.
...
| 희소 보상 | 완전 회복·위험 무기·Run 내 지속 수치 성장 | C12·W11·P02 | 특정 보상이 승패를 독점하는지 평가 |

상품 풀 후보는 무기·방어구·보급품으로 나누되, 아이템상점의 세부 상품군 전문화는 후속 결정한다. 아이템상점의 5개·진열 내 중복 없음·리롤 1G는 [6절](#6-구현-원칙과-다음-콘텐츠)의 시험 규칙이다. 보급품의 최소 재고 보장, 직업 능력치에 맞는 무기 보장, 최종 중복·재방문 갱신·새로고침 비용·매입가는 아직 미정이다. W02처럼 원작 입수 경로가 제한된 항목을 일반 상품으로 넣으면 원작 대비 변경 내역을 남긴다.
...
최종 거래 구성의 제안은 **필수 보급품 슬롯 + 파티 역할 보완 장비 슬롯 + 자유 추첨 슬롯**이다. 상품 후보가 부족할 때는 다른 합법 후보로 대체하고, 그래도 없으면 빈 슬롯을 표시하는 규칙을 후보로 둔다. 상점 진입 시 확정한 상품 ID·수량·가격·변형을 저장하여 로드로 재추첨하지 않는 계약이 필요하다. 아이템상점의 현재 시험 구현은 [6절](#6-구현-원칙과-다음-콘텐츠)을 따르며 최종 전문화·슬롯·가격·새로고침·구매 권한은 [후속 결정](TODO.md#3-2-아이템과-직업) 대상이다.
...
ProjectA에는 직업 역할·필수 보급과 장비 슬롯을 분리한 상점·약초 부가 효과 모델을 **채택 미확정 제안**으로 보충했다. 공격 적중은 [8-5절의 충돌 판정](#8-5-발동과-피격), 장비 슬롯과 장비 부여 스킬 방향은 [2-2절](#2-2-장비-슬롯과-장비-부여-스킬)로 확정했다. 기존 시간차 전투와 상태이상 규칙을 유지하며 원작 완전 성공 부가 효과의 변환·회복 모델·장비 교체/스킬 연결 세부 규칙·개인/공동 재화는 [TODO 3-2절](TODO.md#3-2-아이템과-직업), 경쟁 Snapshot 범위는 [TODO 4-2절](TODO.md#4-2-서비스-구현과-검증)의 사용자 결정 대상으로 둔다. 확인 절차와 미실행 항목은 [남은 확인](TODO.md#3-2-아이템과-직업)에 기록한다.
...
→ 플레이어별 주요 행동 선택 또는 턴 넘기기·복구 상태 저장 성공 후 준비 완료
...
2026-09-22 사용자 요청에 따라 스킬을 선택하지 않고 준비 완료하면 해당 유닛은 AP 행동을 건너뛴다. 스킬 미선택은 장착 스킬이 없어도 유효하며 행동 AP/SAP 비용은 0이다. 선택한 스킬을 취소하면 같은 대기 상태로 돌아간다. SAP 이동만 예약한 경우 이동은 실행하고 SAP 1만 차감한다. 선택한 스킬의 대상·자원 오류를 자동으로 대기로 바꾸지는 않는다.
...
| 대상 사망 | 유닛 대상 공격은 발동 전 대상이 죽으면 공격자의 현재 월드 위치에서 가장 가까운 유효한 생존 적으로 변경. 유닛 접근형은 새 대상에게 접근·재시전하며 추가 AP/SAP 차감 없음. 후보가 없으면 불발 후 복귀 |
...
| 근접 타격 | 일반 단일 근접은 발동 순간 전방 sphere sweep. 검 공격은 활성 구간의 칼날 소켓 궤적 sweep으로 최초 유효 적 한 명에게 1회 적용. 선택한 조준 대상과 실제 피격 대상은 다를 수 있음 |
...
일반 근접·지점 공격 쿼리와 투사체 발사는 C++ 서버가 `WindupSeconds` 종료 시 수행하며 투사체 충돌은 이후 비행 진행마다 검사한다. 검 공격의 `bUseWeaponTrace`는 지정 활성 구간에 유닛별로 결정한 몽타주 에셋 포즈·메시·무기 부착·소켓의 칼날 궤적을 서버에서 샘플링한다. 렌더 메시 갱신·재생 인스턴스 종료에 판정을 종속시키지 않으며 행동 취소·사망·대상 상실은 서버 행동 단계에서 처리한다. 클라이언트 프레임률·애니메이션 알림은 피해를 확정하지 않는다. 검 몽타주·소켓·부착 위치 변경은 충돌 궤적과 적중 순서에도 영향을 주므로 표현만의 변경으로 취급하지 않는다.
...
2026-09-22 사용자 요청에 따라 유효한 근접·투사체 프로필은 기존 `TargetLoss` 저장값과 관계없이 발동 전 가장 가까운 유효 적 재선택을 공통 적용한다. 후보는 같은 전투의 기존 유닛 대상 검사를 통과해야 하며 거리 기준은 공격자의 현재 위치다. 시전 중 변경은 미발동 몽타주·선딜·칼날 궤적을 초기화하고 새 대상 기준으로 다시 진행한다. 지점 공격의 선택 좌표·이미 발사한 투사체·원래 복귀점·차감 비용은 유지하며 스킬을 추가 발동하지 않는다.

| 지원되는 이전 정의 | 초기 라운드 변환 |
|---|---|
| EnemyUnit·Single·제자리 공격 | 투사체, 발동 전 목표 상실 시 가장 가까운 유효 적 재선택 |
| EnemyUnit·Single·`bMoveToTarget` | 실제 유닛 접근형 근접, 발동 전 목표 상실 시 가장 가까운 유효 적 재선택 |
| EnemyUnit·TargetAndSides·`bMoveToTarget` | 대상과 양옆 한 칸씩을 훑는 근접, 발동 전 목표 상실 시 가장 가까운 유효 적 재선택 |
...
변환은 PrimaryAssetId·표시명·AP 비용·authored 공격 위력과 기존 AttackMontage의 시전 표현을 연결하며 나머지 시간·속도는 시험 기본값이다. 명시 프로필의 CastMontage가 있으면 우선 사용한다. 몽타주의 루트 모션·알림으로 이동이나 피해를 확정하지 않으며, `bUseWeaponTrace`만 서버가 몽타주 포즈를 칼날 궤적의 입력으로 사용한다. 기존 GAS 효과·상태효과를 그대로 실행하거나 완전 이식하는 방식이 아니다. 회복 스킬을 다른 효과로 자동 치환하지 않는다.

Snapshot 상대도 같은 `ResolveRoundSkill` 검증을 사용한다. 에셋 ID로 중복을 검사하며 이전 GAS 클래스를 공유하는 서로 다른 명시 스킬은 허용한다.

실전 스킬 목록은 실제 장착 DA만 사용한다. 공통 시험 행동 주입·무장착 대체 공격과 Encounter 진입 시 스킬 풀 자동 추첨을 제거했다. 새 Run의 아군은 `BPDA_DefaulatAttack` 비무장 공격 하나로 시작하고 구매한 DA를 저장된 장착 목록에 추가한다. 기존 Blueprint의 4/5개 장착은 이전 저장 fallback으로 보존하며 새 Run에 자동 지급하지 않는다. 스킬 ID·GAS·타격값은 유지한다. 직업별 최종 밸런스는 별도다.
...
검 공격 `BPDA_swoard_attack`은 피해 50·AP 1·민첩 기반 접근과 시전 종료 후 복귀를 사용한다. `bUseWeaponTrace=true`로 시전 0.23~0.43초 동안 `BladeBase`~`BladeTip` 궤적을 0.005초 간격·반경 4cm의 sphere sweep으로 검사한다. 서버가 `GetAnimationPose`로 유닛별 몽타주 포즈를 계산하여 전용 서버·낮은 프레임률에서도 렌더 메시 갱신에 의존하지 않는다. 칼날에 닿은 최초 생존 적에게 기존 GAS `Data.Damage` 경로로 한 번 피해를 적용하며 빗나감·벽 차폐·사망 취소를 유지한다. 비무장·휩쓸기 판정은 변경하지 않는다.

Paragon Kwang의 `PrimaryAttack_A_Slow` 1.2초와 `PrimaryAttack_A_Slow_Recovery` 0.933333초는 캐릭터 외형 변경 이전부터 사용하던 GKnight·Manny·적용 리타깃을 유지한다. 공격 끝부분과 포즈가 겹치는 복귀 클립의 첫 0.2초를 제외한 총 1.933333초·블렌드 인 0.08초/아웃 0.12초를 유지한다. 검 부착은 Roll 180도·Yaw 0도이며 기존 Boss Swing 사본·Blueprint의 4/5개 기본 스킬·Snapshot 입력의 저장된 스킬을 보존한다. 기본 적은 검 하나와 검 공격만 사용한다. 검 방향·실제 접촉은 [사용자 확인](TODO.md#2-25-파라곤-적용-롤백) 대기다.
