...
계획 UI는 살아 있는 적 공격 대상과 아군 Guard 대상을 구분하고 Wait·GroundAttack에 유닛 대상을 요구하지 않는다. 같은 조작 유닛과 스킬 목록을 유지하는 동안 대상 또는 다른 소유 아군의 사망으로 목록이 갱신되어도 스킬 초안은 유지한다. 적용 전 자원·타일·대상을 검사하며 자신에게 인간 조작이 허용된 모든 생존 유닛에 유효한 계획이 적용되어야 준비 요청이 가능하다. 서버의 소유권·수정 번호·최종 예약 충돌 검사는 유지한다. 이 입력 보완은 준비 해제·예약 경합의 임시 정책을 새로 확정한 것이 아니다.
