...
근접 접근·복귀의 속도 보정은 [8-4절](#8-4-공격-접근과-복귀), SAP 고정 이동 속도는 [8-2절](#8-2-행동-라운드)을 따른다. 비근접 스킬 이동·시전·투사체 속도에는 이번 민첩 보정을 적용하지 않는다. AP·보조 AP는 유지한다. 초기 구현은 라운드 계획 시작에 초기화하고 전체 명령 잠금 시 한 번 차감한다. 한 주요 행동만 예약하며 이후 불발·사망에 비용을 환불하지 않는다. 직업·아이템의 세부 자원 밸런스는 후속 검토다.
