...
현재 구현은 동일 Gameplay의 두 전투 노드를 시간차 라운드 전투로 진행하며 패배 시 Run을 종료한다. 승리마다 5~15G 보상 3개 중 하나를 수령한다. 새 Run의 첫 승리 보상 뒤에는 상점1·상점2·상점3을 고정 제시하고 하나를 선택한다. 상점1·3은 개인 골드로 본인 스킬·회복을 구매하고 상점2는 아이템 구매·리롤을 시험한 뒤 두 번째 전투로 연결한다. 마지막 승리도 보상 수령 뒤 Run을 종료한다.
...
직업 스킬·별도 습득 스킬과 장비 스킬의 관계, 장착·교체 가능 시점·비용, 방패 사용 여부·점유 규칙은 후속 결정한다. 현재 비무장 시작·스킬 상점 구매와 상점2의 아이템 보유·리롤 시험은 유지한다. 아이템 구매는 장착·스킬 부여·메시 표시를 포함하지 않으며 장비 시스템 구현 완료로 간주하지 않는다.
...
각 인카운터의 확정 선택 규칙은 **후보 3개 제시 → 하나 선택 → 선택한 인카운터 진행**이다. 목표 Run은 [2절](#2-목표-run과-현재-구현)의 순서에 따라 PvE 전후에 각각 인카운터를 3회 진행한다. 현재 두 전투 사이에 한 번 방문하는 시험용 상점의 규칙은 다음과 같다.

- 새 Run의 아군 네 직업은 비무장 공격만 소유한다. 기본 적의 검 공격과 기존 Snapshot 스킬은 유지한다.
- 직접 조작 캐릭터별 개인 10G로 시작한다. AI 동료에게 골드를 지급하거나 구매 스킬을 배분하지 않는다.
- 상점1·상점3은 기존 검·원거리·AOE·휩쓸기를 각 1G에 판매한다. 직업 제한 없이 같은 스킬을 캐릭터별 한 번 구매하고 Run 동안 유지한다.
- 상점1·상점3은 HP 전체 회복을 1G에 판매한다. 생존한 본인 캐릭터의 HP를 구매 즉시 최대치까지 회복하며 만피·사망 상태에서는 구매할 수 없다.
- 2026-09-25 상점2 시험은 [무기 CSV](WEAPON_ASSETS.csv)의 방패·탄환·화살·기타를 포함한 전체 295개에서 중복 없이 5개를 제시한다. 원본 에셋 이름과 `가격(G)`을 사용하고 현재 모든 상품·리롤 비용은 1G다. 구매는 개인 Run 아이템 수량에 반영하고 슬롯을 판매 완료로 표시한다. 리롤에서 이전 상품의 재등장을 허용한다. 장착·스킬 부여·메시 표시는 후속 작업이다.
- 협동에서는 원래 본인 소유의 생존 인간 조작 캐릭터만 구매한다. Host의 타인 구매와 AI 전환 캐릭터 구매를 허용하지 않는다. 상점 선택·퇴장은 현재 Host가 결정한다.
- 골드 차감과 스킬 획득·HP 회복 또는 아이템 구매·진열 변경은 함께 저장하며 실패 시 확정하지 않는다. 기존 스킬 상점 저장의 골드·스킬은 보존하고 아이템 상점은 새 Run에만 적용한다. 이전 저장의 아이템 상점 schema 0에는 새 Run 안내를 표시하며 상품·골드를 소급 지급하지 않는다.
...
추가 보상 종류·최종 재입고·확률 가중치·플레이어 간 아이템 거래는 별도 결정한다. 골드 수치와 상점2 상품 추첨은 임시 시험 규칙이며 콘텐츠별 태그 가중치 설계의 완료를 의미하지 않는다. 기존 콘텐츠 태그·GAS 실행 연결을 유지한다.
...
상품 풀 후보는 무기·방어구·보급품으로 나누되, 최종 상점 역할은 후속 결정한다. 상점2의 5개·진열 내 중복 없음·리롤 1G는 [6절](#6-구현-원칙과-다음-콘텐츠)의 시험 규칙이다. 보급품의 최소 재고 보장, 직업 능력치에 맞는 무기 보장, 최종 중복·재방문 갱신·새로고침 비용·매입가는 아직 미정이다. W02처럼 원작 입수 경로가 제한된 항목을 일반 상품으로 넣으면 원작 대비 변경 내역을 남긴다.
...
최종 거래 구성의 제안은 **필수 보급품 슬롯 + 파티 역할 보완 장비 슬롯 + 자유 추첨 슬롯**이다. 상품 후보가 부족할 때는 다른 합법 후보로 대체하고, 그래도 없으면 빈 슬롯을 표시하는 규칙을 후보로 둔다. 상점 진입 시 확정한 상품 ID·수량·가격·변형을 저장하여 로드로 재추첨하지 않는 계약이 필요하다. 상점2의 현재 시험 구현은 [6절](#6-구현-원칙과-다음-콘텐츠)을 따르며 최종 전문화·슬롯·가격·새로고침·구매 권한은 [후속 결정](TODO.md#3-2-아이템과-직업) 대상이다.
