...
| 아이템 효과 | 회복량/수량 데이터·효과·SubAP 소모·HUD 자기 회복 연결 | T13 구현 |
| 적 이동 의사결정 | 공통 이동 후보의 전진 점수와 완료/실패 전이 연결, 기본 공격 임시 가중치 제거 | T13 구현 |
...
| 메뉴 재진입 프리뷰 | Back/X에서 프리뷰 정리, 재진입 시 빈 초안, 배치된 PreviewStage 카메라 유지 구현. Designer/native 반복 검증 | T12 |
| 멀티플레이 | Async PvP Snapshot과 Listen Server Co-op 기획 확정 / 구현 후속. 일부 복제 선언만 있으며 Snapshot Encounter·전체 클라이언트 동기화·다중 PIE는 미검증 | D01 / T14 |
