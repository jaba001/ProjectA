...
전투 준비 취소 중 저장이 실패하면 Host의 **저장 다시 시도**로 Map 복귀를 재시도한다. 기존 확정 저장과 원래 준비 오류를 보존하며, 실패 중 새 전투를 중복 생성하지 않는다. 파티 설정은 Content Browser의 제작 파티 에셋에서 **Validate Assets**로 미리 검사할 수 있다. [복구·사전 검사 절차](Docs/TEST_REPORT.md#18-준비-취소-복구와-파티-데이터-사전-검사)를 따른다.
