...
새 전투 목표는 [라운드 계획·시간차 자동 전투](Docs/GAME_DESIGN.md#8-라운드-계획과-시간차-자동-전투)다. 라운드마다 행동을 예약하고 속도 1 차이당 0.1초 후 실행하며, 실제 접근·복귀와 서버 실시간 피격을 적용한다. 기획 방향만 확정했으며 현재 실행은 아래의 기존 개별 유닛 순차 턴이다. [후속 결정](Docs/TODO.md#8-시간차-자동-전투-후속-결정)과 [검토·검증 계획](Docs/TEST_REPORT.md#12-시간차-자동-전투-기획-검토)을 구분한다.
