...
| 전투 | CommonUI 행동 계획·준비, 합법 대상·자원·타일의 적용 전 검사, 시간차 실행·접근/복귀·근접/지점/투사체 충돌 판정과 DA 시전 몽타주 연결. AI는 장착된 복귀형 Tile 공격도 선택 가능. 사용자 작동 확인 대기 |
...
장착한 스킬의 기존 `AbilityClass → AttackMontage`는 시전 시작 시 재생한다. `bUseRoundDefinition`을 사용하는 DA는 `RoundDefinition.CastMontage`로 재생할 몽타주를 우선 지정할 수 있다. 몽타주 길이·알림은 피해 판정이나 복귀 시각을 변경하지 않으며 메시·AnimBP의 Skeleton/Slot 연결이 필요하다. [확인 절차와 실패 로그](Docs/TEST_REPORT.md#21-da-시전-몽타주-연결)를 따른다.

복귀형 공격은 도착 후 행동 전 방향으로 돌아선다. 정상 복귀·시간 초과 복원·제자리 완료에 적용하며 [복귀 방향 확인](Docs/TEST_REPORT.md#21-4-공격-후-복귀-방향-복원)은 사용자 검증 대기다.

최초 사용자 확인은 [싱글 Run 통합 절차](Docs/TEST_REPORT.md#1-직접-플레이-확인)의 새 게임·첫 전투·결과·상점·두 번째 전투와 설정 변경·복원이다. 이후 2인 협동·4인 협동 순서로 진행한다. 최신 준비 점검은 Editor 컴파일·정적 검사까지 완료했으며 작동 검증은 대기다.
