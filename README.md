...
기본 전투는 [라운드 계획·시간차 자동 전투](Docs/GAME_DESIGN.md#8-라운드-계획과-시간차-자동-전투)다. 각 라운드의 행동을 함께 계획하고 속도 1 차이당 0.1초 후 실행한다. 실제 접근·복귀, 유닛 이동 비충돌, [서버의 공격 충돌 판정](Docs/GAME_DESIGN.md#8-5-발동과-피격)을 사용한다. 공격은 실제 피격체에 닿아야 하며 별도 명중 확률·성공 슬롯 판정을 사용하지 않는다. 기존 순차 턴 실행은 제거했으며 기존 Blueprint 참조용 클래스·프로퍼티만 남긴다. 일부 [임시 구현 정책](Docs/TODO.md#3-1-전투-규칙)은 사용자 확인 대기다.
...
| 전투 | CommonUI 행동 계획·준비, 합법 대상·자원·타일의 적용 전 검사, 시간차 실행·접근/복귀·근접/지점/투사체 충돌 판정과 DA 시전 몽타주 연결. 발동 후 서버 몽타주 종료를 기다려 복귀하며 도착 시 원래 방향 복원. AI는 장착된 복귀형 Tile 공격도 선택 가능. 사용자 작동 확인 대기 |
...
메인메뉴의 **개발용 협동**에서 2~4인 방 생성·IPv4 주소 참가·준비·Host 시작을 지원한다. 참가자마다 궁수 한 명을 배정하며 실제 연결은 Unreal Listen Server를 사용한다. Shipping에서는 비활성이다. Steam 초대·로그인·관리 Run 재개 UI는 별도이며, 새 UI의 작동 검증은 대기다.
...
현재 사용자가 작성한 공격 DA는 기본 공격이며 사용자가 저장한 이름·ID를 유지한다. 나머지 공격은 미작성 상태를 보존하고 새 수치·몽타주를 채우지 않는다. 제작 DA 7개는 `Blueprint/DataAsset` 아래 `Skills`·`Parties`·`Encounters`·`SkillPools`·`Snapshots`로 분류한다. 객체 이름·값·ID를 유지하고 기존 저장 파일의 소프트 경로는 정확한 패키지 리디렉션으로 연결한다. [폴더 구성](Docs/PROJECT_PLAN.md#gameplay-에셋과-배치)과 [전체 시전·이동 후 확인](Docs/TEST_REPORT.md#21-5-전체-시전-대기와-da-폴더-정리)을 따른다.
