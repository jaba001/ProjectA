...
def create_catalog():
    ...
    for name, suffix in [("Head", "Head"), ("Torso", "Chest"), ("Arms", "Arms"), ("Hands", "Hands"), ("Legs", "Legs"), ("Feet", "Feet")]:
        ...
        part.set_editor_property("mesh", load(BODY + "/Body_parts/SK_Manny_" + suffix))
...
def configure_profession(profession, catalog):
    ...
    for ending in ["Unit", "SnapshotOpponent"]:
        ...
        for field in ["skeletal_mesh_asset", "anim_class", "relative_location", "relative_rotation", "relative_scale3d", "physics_asset_override"]:
            mesh.set_editor_property(field, source_mesh.get_editor_property(field))
        mesh.set_editor_property("override_materials", [])
        defaults.set_editor_property("round_montage_overrides", dict(source.get_editor_property("round_montage_overrides")))
        ...
        attach_weapon(blueprint, "Sword", source_sword.get_editor_property("static_mesh"), str(HELPER.get_weapon_attachment(source_blueprint, "Sword")), grip)
        unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
        save(blueprint)
...
def configure():
    ...
    configure_profession("Warrior", catalog)
    report = {"catalog": catalog.get_path_name(), "slots": len(catalog.get_editor_property("slots")), "items": len(catalog.get_editor_property("items")), "excluded_incompatible_meshes": excluded, "source_assets_copied": 0, "gameplay_test": "not run"}
